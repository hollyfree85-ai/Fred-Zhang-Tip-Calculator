...
function ownerTableRow(s,name){
  ...
  if(shift==='LONG')row.totalAM=row.total24;
...
function ownerTableLinked24(row){return row.shift==='AM' && !!ownerTableSession?.bartender24;}
function ownerTableEdit(name,field,value){
  ...
  if(row.role==='Server'){
    if(field==='totalAM' && row.shift==='LONG'){row.total24=value;changed.total24=value;}
    if(field==='totalAM' && row.shift==='AM' && !ownerTableLinked24(row)){row.grand=value;changed.grand=value;}
    ...
    if(row.shift==='LONG')row.totalAM=row.total24;
    if(row.shift==='AM' && !ownerTableLinked24(row))row.totalAM=row.grand;
  }
...
function ownerTableRender(){
  ...
  host.innerHTML=session.rows.map(row=>{
    ...
    const money=(key,disabled=false,note='')=>`<td><input type="number" min="0" step="0.01" inputmode="decimal" placeholder="0.00" data-owner-table-field="${key}" data-employee="${employee}" value="${esc(row[key]??'')}" aria-label="${esc(row.name)} ${key}" ${disabled?'disabled':''}>${note?`<small>${note}</small>`:''}</td>`;
    const check=(key,label,disabled=false)=>`<td class="owner-table-check"><input type="checkbox" data-owner-table-field="${key}" data-employee="${employee}" ${row[key]?'checked':''} ${!server||disabled?'disabled':''} aria-label="${esc(row.name)} ${label}"></td>`;
    const amDisabled=row.shift==='PM' || (!server && !['DOUBLE','LONG'].includes(row.shift));
    return `<tr><th scope="row"><b>${esc(row.name)}</b><small>${row.role}</small></th><td><select data-owner-table-field="shift" data-employee="${employee}" aria-label="${esc(row.name)} shift"><option value="">Select shift</option>${['AM','PM','DOUBLE','LONG'].map(v=>`<option value="${v}" ${row.shift===v?'selected':''}>${v==='LONG'?'Long Shift':v==='DOUBLE'?'Double Shift':v}</option>`).join('')}</select></td>${money('grand')}${check('barPM','BAR Sales PM',row.shift==='AM')}${money('totalAM',amDisabled,server && row.shift==='LONG'?'Linked to Total 2–4':server && row.shift==='AM'&&!ownerTableLinked24(row)?'Linked to Grand Total':'')}${check('barAM','BAR Sales AM',row.shift==='LONG')}${money('total24',!server)}${check('bar24','BAR Sales 2–4')}${money('paid')}${money('cardFee')}${money('cash')}${money('meal')}</tr>`;
...
function ownerTableBuildBatch(source,session){
  ...
  for(const [name,changes] of Object.entries(session.dirty)){
    ...
    const d=s.drafts[name] ||= hv1BlankDraft(name);
    if(!['AM','PM','DOUBLE','LONG'].includes(row.shift))throw new Error('Select a shift for '+name+'.');
    ...
    hv1SetValue(d,'hPosition',row.role);hv1SetValue(d,'hShift',row.shift);
    ...
    for(const [key,field] of Object.entries(OWNER_TABLE_MONEY)){
      if(!(key in changes))continue;
      if(key==='totalAM' && !['DOUBLE','LONG'].includes(row.shift))continue;
    ...
    if(row.role==='Server'){
      ...
      if('totalAM' in changes && ['AM','DOUBLE'].includes(row.shift))write('AM',row.totalAM);
      ...
      if(row.shift!=='LONG')hv1SetValue(d,'hAmBar',hv1BarExcluded(amKey,s,name)?'no':'yes');
      hv1SetValue(d,'hPmBar',hv1BarExcluded('PM',s,name)?'no':'yes');
...
window.ownerTableOpen=async function(){
  ...
  ownerTableSession={date,uid,bartender24:!!state.bar['2PM_4PM'].bartender,rows:(state.team||[]).map(name=>ownerTableRow(state,name)),dirty:{}};
...
function hv1SyncBarChoices(s,name){
  ...
  if(!d || hv1DraftRole(d)==='bartender')return;
  const shift=hv1DraftShift(d);
  const pairs=[['hAmBar',shift==='AM'?hv1BarSalesFieldMap(shift,s,name).hGrandTotal:'AM'],['hPmBar','PM']];
...
window.logout = async function(){
  ownerTableSession=null;++ownerTableRequest;$("ownerTableBody").innerHTML="";
  hv1RestoreSmallReportHome();
...
async function enableBackgroundPush(){
  ...
  const swReg=await navigator.serviceWorker.register("./service-worker-v13839.js?v=13839",{updateViaCache:"none"});
