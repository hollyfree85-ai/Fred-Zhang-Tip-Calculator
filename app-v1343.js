...
function howField(label,id,value,type="text",placeholder=""){
  return `<div class="how-field"><label>${label}</label><input id="${id}" type="${type}" value="${String(value??"").replace(/"/g,"&quot;")}" placeholder="${placeholder}"></div>`;
}
...
function renderHourlyWizard(){
  ...
  if(hourlyWizardStep===6){
    if(hourlyWizardState.position==="Bartender"){
      body=`<h3 class="how-title">Bar Tip Out</h3><p class="how-sub">Use the existing bartender calculator for the combined server tip out.</p>
        <div class="how-card"><div class="how-info">Bartender calculation fields will open below this wizard for this step.</div></div>${howNav()}`;
    }else{
  ...
  // Bartender-only legacy calculator is shown only on Bar Tip Out step.
  const bt=$("hBartenderBarReceivedWrap");
  if(bt){
    const show=hourlyWizardStep===6 && hourlyWizardState.position==="Bartender";
    bt.classList.toggle("hourlyLegacyHidden",!show);
  }
...
function captureHourlyWizard(){
  if(hourlyWizardStep===1){
    howSet("hDate",$("howDate")?.value||howVal("hDate"));
    howSet("hEmployee",$("howEmployee")?.value||howVal("hEmployee"));
    howSet("hPosition",hourlyWizardState.position);
    ...
    howSet("hShift",targetShift);
    syncHourlyShiftUI?.();
    syncBartenderBarReceivedField?.();
  }else if(hourlyWizardStep===2){
    if(hourlyWizardState.shift==="DOUBLE"){
      howSet("hAmIn",$("howAmIn")?.value||""); howSet("hAmOut",$("howAmOut")?.value||"");
      howSet("hPmIn",$("howPmIn")?.value||""); howSet("hPmOut",$("howPmOut")?.value||"");
    }else{
      howSet("hIn",$("howIn")?.value||""); howSet("hOut",$("howOut")?.value||"");
    }
  }else if(hourlyWizardStep===3){
    howSet("hBusserAM",hourlyWizardState.busserAM);
  }else if(hourlyWizardStep===4){
    howSet("hGrandTotal",$("howGrand")?.value||0);
    if(hourlyWizardState.shift==="DOUBLE") howSet("hTotalAM",$("howTotalAM")?.value||0);
    else howSet("hTotalAM",0);
  }else if(hourlyWizardStep===5){
    howSet("hPaidTip",$("howPaid")?.value||0); howSet("hCardFee",$("howCardFee")?.value||0);
    howSet("hCashTip",$("howCash")?.value||0); howSet("hMeal",$("howMeal")?.value||0);
  }else if(hourlyWizardStep===6 && hourlyWizardState.position!=="Bartender"){
    howSet("hAmBar",$("howAmBar")?.checked?"yes":"no");
    howSet("hPmBar",$("howPmBar")?.checked?"yes":"no");
  }
}

function bindHourlyWizard(){
  document.querySelectorAll("[data-how-choice]").forEach(b=>b.onclick=()=>{
  ...
  if($("howNext")) $("howNext").onclick=()=>{
    ...
    if(hourlyWizardStep===7){
      hourlyWizardStep=1;
      // clear manual-entry values but keep roster/date intact
      ["hIn","hOut","hAmIn","hAmOut","hPmIn","hPmOut","hGrandTotal","hTotalAM","hPaidTip","hCardFee","hCashTip","hMeal"].forEach(id=>{if($(id))$(id).value=id==="hMeal"?"0":"0";});
...
setTimeout(()=>{ if($("hourlyOriginalWizard")) renderHourlyWizard(); },900);
