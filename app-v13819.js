...
// BAR-only repair on the user-supplied V13.8.14 baseline.
// Cumulative checkpoints, not three independent sets of sales.
...
window.calculateBartenderBarTipOut=function calculateBartenderBarTipOut(){
  ...
  const lessAM=managed?snapshot.lessAM:(type==="AM" ? 0 : Number($("hBtPrevAMInput")?.value||0));
  const less24=managed?snapshot.less24:(type==="PM" ? Number($("hBtPrev24Input")?.value||0) : 0);
...
// V13.8.14 Small Report fullscreen one-screen viewport + freeze pane + close.
