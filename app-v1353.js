...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow){
    // Capture the visible Step-6 fields first and use THIS state as the source of truth.
    // Do not let the hidden legacy calculator overwrite the visible result.
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.5.3 hard guard: visible Step-6 result stays authoritative.
...
function captureHowBartenderDom(){
  ...
  for(let i=1;i<=9;i++){
    d.servers[i-1]={
      name:String($(`howBtName${i}`)?.value||"").trim(),
      grandTotal:String($(`howBtGrand${i}`)?.value||"").trim()
    };
  }
      ...
      <div class="how-card">
        <div class="how-summary">
          ...
          ${hourlyWizardState.position==="Bartender"
            ? `<div><small>Bar Tip Out Received</small><b>${howMoney(howBartenderFormula().finalReceived)}</b></div>`
            : ""}
        </div>
...
// V13.5.3 Calculate V01 + fresh page fix.
