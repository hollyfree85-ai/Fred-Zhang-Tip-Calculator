...
// V13.8.17A — reusable Show Password checkbox for all password/PIN inputs.
...
// V13.8.17A reliable team-card click handling (desktop/tablet/mobile).
...
window.openHourlyWorkspacePanel=function(name){
  // V13.8.17AA: Hourly V01 is no longer an isolated sub-app.
...
function listenEmployeeHistoricalReports(){
  ...
  unsubs.push(onSnapshot(qh,snap=>{
    ...
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.17A hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.17A Firestore rules.</div>`;
    }
  }
}
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    alert(`Delete failed: ${e.code||e.message}\n\nIf this says permission-denied, publish the V13.8.17A Firestore rules.`);
  }
};
...
function pdfReportContent(r,index,total){
  ...
  const right=[
    ...
    ["CASH TIP",pdfMoney(r.cashTip)],
    ["GRAND TOTAL TO EMPLOYEE",pdfMoney(employeeGrandTotal)]
  ];
  ...
  c+="BT /F2 13 Tf 28 101 Td (TOTAL PAID OUT) Tj ET\n";
  c+=`BT /F2 23 Tf 28 72 Td (${pdfEscape(pdfMoney(paidOut))}) Tj ET\n`;
  c+="BT /F2 11 Tf 170 101 Td (CASH TIP) Tj ET\n";
  c+=`BT /F2 20 Tf 170 74 Td (${pdfEscape(pdfMoney(r.cashTip))}) Tj ET\n`;
  c+="BT /F2 13 Tf 292 101 Td (GRAND TOTAL) Tj ET\n";
  c+=`BT /F2 23 Tf 292 72 Td (${pdfEscape(pdfMoney(employeeGrandTotal))}) Tj ET\n`;
  c+="BT /F1 8 Tf 292 58 Td (Total Total Before Meal + Cash Tip) Tj ET\n";

  const status=String(r.status||"MONEY READY").toUpperCase();
  c+="BT /F2 9 Tf 430 112 Td (FINAL / MONEY READY) Tj ET\n";
  c+=`BT /F1 7.5 Tf 430 101 Td (Status: ${pdfEscape(status)}) Tj ET\n`;

  c+="BT /F2 8.5 Tf 450 58 Td (EMPLOYEE SIGNATURE) Tj ET\n";
  c+="0.7 w 450 12 m 580 12 l 580 52 l 450 52 l 450 12 l S\n";
  c+=pdfSignatureCommands(r.pickupSignature,454,16,122,32);

  if(Array.isArray(r.pickupSignature?.strokes) && r.pickupSignature.strokes.length){
    c+="BT /F1 7 Tf 450 4 Td (SIGNED) Tj ET\n";
  }
...
window.closeSmallReportDetail=function(){
  ...
  // Always return to V1 Small Report — never to legacy Hourly V01.
  if(hourlyV1Mode){
    document.body.classList.add("hourly-v1-mode","hourly-v1-small-report");
    $("hourlyV1Workspace")?.classList.remove("hidden");
    $("hv1SmallReportBox")?.classList.remove("hidden");
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.17A hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.17A Busser split. Preserve the existing V01 total busser formula,
  ...
  // V13.8.17A payout rule: employee payout is Total Before Meal minus Meal.
...
// V13.8.17A Calculate V01 + fresh page fix.

// V13.8.17A: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.17A Employee Clear All fix.

// V13.8.17A Small Report tab + signature XLS.

// V13.8.17A dedicated Hourly Adjustment sub-app.

// V13.8.17A fresh page generated from current index.

// V13.8.17A Edit finalized Hourly report from Small Report.

// V13.8.17A Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.17A staff Remember Me with Firebase local/session persistence.

// V13.8.17A Main App button removed from Hourly workspace.

// V13.8.17A Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.17A Remember Me for dedicated Hourly login.

// V13.8.17A Small Report server signature + SMS.

// V13.8.17A Employee phone directory for Small Report SMS.

// V13.8.17A recovered employee phone directory + reliable SMS composer.

// V13.8.17A Add/Edit employee + phone directory.

// V13.8.17A Hourly Adjustment V1 batch workspace.

// V13.8.17A login role selector responsive UI fix.

// V13.8.17A Delete Team.

// V13.8.17A Delete individual Hourly V1 team member.

// V13.8.17A Cloud-backed Hourly V1 drafts.

// V13.8.17A live bar calculation after partial Grand Total save.

// V13.8.17A Team BAR Center with AM / 2-4 / PM automation.

// V13.8.17A show password checkboxes.

// V13.8.17A reliable team-card and BAR server input interactions.

// V13.8.17A separate Team Board and employee editor views.

// V13.8.17A persist BAR state to hourlyV1Batches cloud document.

// V13.8.17A secure Clear All, Owner Undo and permanent deletion.

// V13.8.17A BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.17A: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.17A: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.17A Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.17A employee soft-delete ownership uses Firebase UID.

// V13.8.17A finalized edit hydration, autosave, quick Team Board/BAR navigation.

// V13.8.17A: direct final editing and two-way BAR/server sync.

// V13.8.17A larger UI, complete signed PDF/SMS, Small Report Grand Total.

// V13.8.17A Grand Total = Total Before Meal + Cash Tip.

// V13.8.17A larger Small Report; frozen Date + Employee columns.

// V13.8.17A simple Small Report list, detail modal, Busser AM/PM split.

// V13.8.17A Small Report stays in Hourly V1; employee detail is a true modal; history retained.

// V13.8.17A Small Report always visible in Hourly V1.

// V13.8.17A restores full management tabs when logging in through Hourly V01.
