...
// V13.8.17 — reusable Show Password checkbox for all password/PIN inputs.
...
function hv1RenderBarSummaryOnly(state=hv1Load()){
  ...
  host.innerHTML=`<h4>Server Bar Tip Out Summary</h4>
    ...
    ${allServers.map(name=>{
      ...
      const prevCum=valid24?gt24:amGT;
      const validPM=pmGT>0&&pmGT>=prevCum;
...
// V13.8.17 reliable team-card click handling (desktop/tablet/mobile).
...
function listenEmployeeHistoricalReports(){
  ...
  unsubs.push(onSnapshot(qh,snap=>{
    ...
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.17 hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.17 Firestore rules.</div>`;
    }
  }
}
async function archiveCheckTipSheet(sheet){
  ...
  if(role==="employee"){
    ...
    const sameName=String(sheet.employeeName||"").trim()===mine;
    const sameUid=String(sheet.employeeUid||sheet.submittedByUid||"")===String(currentUser.uid||"");
    if(!sameName && !sameUid){alert("You can delete only your own Check Tip status.");return false;}
  }
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    alert(`Delete failed: ${e.code||e.message}\n\nIf this says permission-denied, publish the V13.8.17 Firestore rules.`);
  }
};
...
window.undoDeletedCheckTip=async function(archiveId){
  ...
  try{await deleteDoc(doc(db,"deletedItems",archiveId));await loadDeletedItems();await loadTipCheckSheets();alert("Deleted Check Tip restored.");}
...
window.undoAllDeletedCheckTips=async function(){
  ...
  try{
    ...
    await loadDeletedItems();await loadTipCheckSheets();alert("All deleted Check Tip records restored.");
...
window.permanentDeleteAllCheckTips=async function(){
  ...
  try{
    ...
    await loadDeletedItems();await loadTipCheckSheets();alert("All deleted Check Tip records permanently deleted.");
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.17 hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.17 payout rule: employee payout is Total Before Meal minus Meal.
...
// V13.8.17 Calculate V01 + fresh page fix.

// V13.8.17: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.17 Employee Clear All fix.

// V13.8.17 Small Report tab + signature XLS.

// V13.8.17 dedicated Hourly Adjustment sub-app.

// V13.8.17 fresh page generated from current index.

// V13.8.17 Edit finalized Hourly report from Small Report.

// V13.8.17 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.17 staff Remember Me with Firebase local/session persistence.

// V13.8.17 Main App button removed from Hourly workspace.

// V13.8.17 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.17 Remember Me for dedicated Hourly login.

// V13.8.17 Small Report server signature + SMS.

// V13.8.17 Employee phone directory for Small Report SMS.

// V13.8.17 recovered employee phone directory + reliable SMS composer.

// V13.8.17 Add/Edit employee + phone directory.

// V13.8.17 Hourly Adjustment V1 batch workspace.

// V13.8.17 login role selector responsive UI fix.

// V13.8.17 Delete Team.

// V13.8.17 Delete individual Hourly V1 team member.

// V13.8.17 Cloud-backed Hourly V1 drafts.

// V13.8.17 live bar calculation after partial Grand Total save.

// V13.8.17 Team BAR Center with AM / 2-4 / PM automation.

// V13.8.17 show password checkboxes.

// V13.8.17 reliable team-card and BAR server input interactions.

// V13.8.17 separate Team Board and employee editor views.

// V13.8.17 persist BAR state to hourlyV1Batches cloud document.

// V13.8.17 secure Clear All, Owner Undo and permanent deletion.

// V13.8.17 BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.17: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.17: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.17 Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.17 employee soft-delete ownership uses Firebase UID.

// V13.8.17 finalized edit hydration, autosave, quick Team Board/BAR navigation.

// V13.8.17: direct final editing and two-way BAR/server sync.

// V13.8.17 larger UI, complete signed PDF/SMS, Small Report Grand Total.

// V13.8.17 Grand Total = Total Before Meal + Cash Tip.

// V13.8.17 Small Report fullscreen one-screen viewport + freeze pane + close.

// V13.8.17 = V13.8.14 UI restored; BAR cumulative + Deleted/Undo refresh fixes only.
