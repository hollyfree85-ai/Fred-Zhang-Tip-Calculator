...
// V13.8.17C — reusable Show Password checkbox for all password/PIN inputs.
...
// V13.8.17C reliable team-card click handling (desktop/tablet/mobile).
...
window.openHourlyWorkspacePanel=function(name){
  // V13.8.17CA: Hourly V01 is no longer an isolated sub-app.
...
function listenEmployeeHistoricalReports(){
  ...
  unsubs.push(onSnapshot(qh,snap=>{
    ...
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.17C hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.17C Firestore rules.</div>`;
    }
  }
}
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    alert(`Delete failed: ${e.code||e.message}\n\nIf this says permission-denied, publish the V13.8.17C Firestore rules.`);
  }
};
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.17C hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.17C Busser split. Preserve the existing V01 total busser formula,
  ...
  // V13.8.17C payout rule: employee payout is Total Before Meal minus Meal.
...
// V13.8.17C Calculate V01 + fresh page fix.

// V13.8.17C: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.17C Employee Clear All fix.

// V13.8.17C Small Report tab + signature XLS.

// V13.8.17C dedicated Hourly Adjustment sub-app.

// V13.8.17C fresh page generated from current index.

// V13.8.17C Edit finalized Hourly report from Small Report.

// V13.8.17C Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.17C staff Remember Me with Firebase local/session persistence.

// V13.8.17C Main App button removed from Hourly workspace.

// V13.8.17C Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.17C Remember Me for dedicated Hourly login.

// V13.8.17C Small Report server signature + SMS.

// V13.8.17C Employee phone directory for Small Report SMS.

// V13.8.17C recovered employee phone directory + reliable SMS composer.

// V13.8.17C Add/Edit employee + phone directory.

// V13.8.17C Tip Calculation batch workspace.

// V13.8.17C login role selector responsive UI fix.

// V13.8.17C Delete Team.

// V13.8.17C Delete individual Tip Calculation team member.

// V13.8.17C Cloud-backed Tip Calculation drafts.

// V13.8.17C live bar calculation after partial Grand Total save.

// V13.8.17C Team BAR Center with AM / 2-4 / PM automation.

// V13.8.17C show password checkboxes.

// V13.8.17C reliable team-card and BAR server input interactions.

// V13.8.17C separate Team Board and employee editor views.

// V13.8.17C persist BAR state to hourlyV1Batches cloud document.

// V13.8.17C secure Clear All, Owner Undo and permanent deletion.

// V13.8.17C BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.17C: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.17C: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.17C Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.17C employee soft-delete ownership uses Firebase UID.

// V13.8.17C finalized edit hydration, autosave, quick Team Board/BAR navigation.

// V13.8.17C: direct final editing and two-way BAR/server sync.

// V13.8.17C larger UI, complete signed PDF/SMS, Small Report Grand Total.

// V13.8.17C Grand Total = Total Before Meal + Cash Tip.

// V13.8.17C larger Small Report; frozen Date + Employee columns.

// V13.8.17C simple Small Report list, detail modal, Busser AM/PM split.

// V13.8.17C Small Report stays in Tip Calculation; employee detail is a true modal; history retained.

// V13.8.17C Small Report always visible in Tip Calculation.

// V13.8.17C restores full management tabs when logging in through Hourly V01.

// V13.8.17C professional non-overlapping PDF employee report layout.

// V13.8.17C removes Hourly V01 login and renames Hourly V1 to Tip Calculation.
