...
// V13.8.24-P15 — reusable Show Password checkbox for all password/PIN inputs.
...
// V13.8.24-P15 reliable team-card click handling (desktop/tablet/mobile).
...
window.openHourlyWorkspacePanel=function(name){
  // V13.8.24-P15A: Hourly V01 is no longer an isolated sub-app.
...
onAuthStateChanged(auth, async user=>{
  ...
  try{
    ...
    showApp();
    // V13.8.24-P15: every successful authenticated login gets a short music sting + voice welcome.
...
function listenEmployeeHistoricalReports(){
  ...
  const fail=e=>{
    ...
    if(host && !window.__employeeHistoricalHourlyReports?.length){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.24-P15 hourlyReports employee-read rule.</span></div>`;
    }
  };
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.24-P15 Firestore rules.</div>`;
    }
  }
}
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    alert(`Delete failed: ${e.code||e.message}\n\nIf this says permission-denied, publish the V13.8.24-P15 Firestore rules.`);
  }
};
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.24-P15 hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.24-P15 Busser split. Preserve the existing V01 total busser formula,
  ...
  // V13.8.24-P15 authoritative minimum-hourly adjustment rule.
...
// V13.8.24-P15 Calculate + fresh page fix.

// V13.8.24-P15: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.24-P15 Employee Clear All fix.

// V13.8.24-P15 Daily Report tab + signature XLS.

// V13.8.24-P15 dedicated Hourly Adjustment sub-app.

// V13.8.24-P15 fresh page generated from current index.

// V13.8.24-P15 Edit finalized Hourly report from Daily Report.

// V13.8.24-P15 Daily Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.24-P15 staff Remember Me with Firebase local/session persistence.

// V13.8.24-P15 Main App button removed from Hourly workspace.

// V13.8.24-P15 Daily Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.24-P15 Remember Me for dedicated Hourly login.

// V13.8.24-P15 Daily Report server signature + SMS.

// V13.8.24-P15 Employee phone directory for Daily Report SMS.

// V13.8.24-P15 recovered employee phone directory + reliable SMS composer.

// V13.8.24-P15 Add/Edit employee + phone directory.

// V13.8.24-P15 Tip Calculation batch workspace.

// V13.8.24-P15 login role selector responsive UI fix.

// V13.8.24-P15 Delete Team.

// V13.8.24-P15 Delete individual Tip Calculation team member.

// V13.8.24-P15 Cloud-backed Tip Calculation drafts.

// V13.8.24-P15 live bar calculation after partial Grand Total save.

// V13.8.24-P15 Team BAR Center with AM / 2-4 / PM automation.

// V13.8.24-P15 show password checkboxes.

// V13.8.24-P15 reliable team-card and BAR server input interactions.

// V13.8.24-P15 separate Team Board and employee editor views.

// V13.8.24-P15 persist BAR state to hourlyV1Batches cloud document.

// V13.8.24-P15 secure Clear All, Owner Undo and permanent deletion.

// V13.8.24-P15 BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.24-P15: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.24-P15: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.24-P15 Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.24-P15 employee soft-delete ownership uses Firebase UID.

// V13.8.24-P15 finalized edit hydration, autosave, quick Team Board/BAR navigation.

// V13.8.24-P15: direct final editing and two-way BAR/server sync.

// V13.8.24-P15 larger UI, complete signed PDF/SMS, Daily Report Grand Total.

// V13.8.24-P15 Grand Total = Total Before Meal + Cash Tip.

// V13.8.24-P15 larger Daily Report; frozen Date + Employee columns.

// V13.8.24-P15 simple Daily Report list, detail modal, Busser AM/PM split.

// V13.8.24-P15 Daily Report stays in Tip Calculation; employee detail is a true modal; history retained.

// V13.8.24-P15 Daily Report always visible in Tip Calculation.

// V13.8.24-P15 restores full management tabs when logging in through Hourly V01.

// V13.8.24-P15 professional non-overlapping PDF employee report layout.

// V13.8.24-P15 removes Hourly V01 login and renames Hourly V1 to Tip Calculation.

// V13.8.24-P15 clean role login buttons and simplified staff dashboard.

// V13.8.24-P15 responsive all-device layout; owner credential UI does not expose plaintext Firebase passwords.

// V13.8.24-P15 cashier role/UI removed.

// V13.8.24-P15 stock-style Analytics chart page.

// V13.8.24-P15 password-protected per-person delete + finalized report restore.

// V13.8.24-P15 Future UI styling / mobile readability.

// V13.8.24-P15 password-gated deletes + Owner Deleted/Undo recovery.

// V13.8.24-P15 successful-login welcome voice + original WebAudio music sting.

// V13.8.24-P15 primes audio/speech during actual login gesture.

setTimeout(()=>{fzOpenGuestReportPortalFromUrl().catch(()=>{});},50);
// V13.8.24-P15: Employee report-only experience + private no-signup portal.

// V13.8.24-P15 adjustment examples:
