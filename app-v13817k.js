...
// V13.8.17K — reusable Show Password checkbox for all password/PIN inputs.
...
function hv1CapturePage(markSkipped=false){
  ...
  hv1SyncServerDraftToBar(s,hv1EditingEmployee);
...
async function hv1HydrateDraftFromFinalReport(name){
  ...
  // Once this build has hydrated the finalized report, preserve every later
  // autosaved edit (including intentionally changing a value to 0).
...
function hv1SyncServerDraftToBar(s,name){
  ...
  // AM-only: BAR AM Grand Total = employee Grand Total.
...
function hv1SyncBarEntryToServerDraft(s,key,name,value){
  ...
  if(key==="AM"){
    d.values.hAmBar=has?"yes":"no"; d.entered.hAmBar=true;
    if(shift==="AM"){
      d.values.hGrandTotal=has?String(n):d.values.hGrandTotal||"";
      if(has)d.entered.hGrandTotal=true;
    }else if(shift==="DOUBLE"){
...
window.hv1DeleteEmployee=async function(name){
  ...
  }catch(e){console.warn("Archive Team Board employee:",e);}
...
window.hv1RestoreEmployeeFinal=async function(name){
  ...
  if(!recoveredReport && name==="Sarah Kibler" && date==="2026-09-05"){
    recoveredReport={
      id:"",
      date:"2026-09-05",
      employee:"Sarah Kibler",
      position:"Bartender",
      shift:"AM",
      busserAM:"N/A",
      hours:{hourIn:"11:04",hourOut:"16:27",hourInAM:"11:04",hourOutAM:"16:27"},
      totalHoursWork:5.38,
      grandTotal:879.94,
      totalAM:879.94,
      totalPM:0,
      ...
      barTipOut:0,
      bartenderShift:"AM",
      serverSalesSummary:5355.25,
      barTipOutReceived:32.13,
      totalBeforeMeal:128.49,
      cashTip:5.00,
      meal:0,
      totalPaidOut:128.49,
      grandTotalTip:133.49,
      status:"money_ready",
      recoverySource:"verified_pdf_2026_09_05"
    };
  }
...
// V13.8.17K reliable team-card click handling (desktop/tablet/mobile).
...
window.openHourlyWorkspacePanel=function(name){
  // V13.8.17KA: Hourly V01 is no longer an isolated sub-app.
  // It opens the complete Manager/Owner dashboard and defaults to Hourly Adjustment V01.
...
window.playSuccessfulLoginWelcome=function(){
  ...
  setTimeout(speakLoginWelcome,330);
...
onAuthStateChanged(auth, async user=>{
  ...
  try{
    ...
    showApp();
    // V13.8.17K: every successful authenticated login gets a short music sting + voice welcome.
...
function listenEmployeeHistoricalReports(){
  ...
  unsubs.push(onSnapshot(qh,snap=>{
    ...
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.17K hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.17K Firestore rules.</div>`;
    }
  }
}
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    alert(`Delete failed: ${e.code||e.message}\n\nIf this says permission-denied, publish the V13.8.17K Firestore rules.`);
  }
};
...
window.restoreDeletedItem=async function(archiveId){
  ...
  const ok=await requireCurrentAccountPassword("Restore Deleted Item","OWNER PASSWORD REQUIRED.");
  if(!ok)return;
  try{
    const s=x.snapshot||{};
    if(x.itemType==="hourly_report"){
      await setDoc(doc(db,"hourlyReports",x.itemId),s);
    }else if(x.itemType==="submission"){
      await setDoc(doc(db,"submissions",x.itemId),s);
    }else if(x.itemType==="user_profile"){
      const clean={...s};delete clean.uid;
      clean.active=s.active!==false;clean.deletedAt=null;clean.deletedBy="";
      await setDoc(doc(db,"users",x.itemId),clean,{merge:true});
    }else if(x.itemType==="team_employee"){
      const date=x.date||s.date||todayLocal();
      const key=`fz_hourly_v1_${date}`;
      const state=JSON.parse(localStorage.getItem(key)||'{"team":[],"drafts":{},"bar":{}}');
      state.team=state.team||[];state.drafts=state.drafts||{};
      const name=x.employeeName||s.name;
      if(name&&!state.team.includes(name))state.team.push(name);
      if(name)state.drafts[name]=s.draft||{};
      localStorage.setItem(key,JSON.stringify(state));
      if(date===hv1DateValue()){await hv1CloudSave(state);hv1RenderCards();}
    }else if(x.itemType==="check_tip"){
      // Check Tip data is soft-hidden by the archive record; removing archive restores visibility.
    }
    await deleteDoc(doc(db,"deletedItems",archiveId));
    await loadDeletedItems();
    if(x.itemType==="hourly_report"){renderSmallReport();}
    alert("Deleted item restored.");
...
window.undoAllDeletedCheckTips=async function(){
  const passwordOk=await requireCurrentAccountPassword("Undo All Deleted","OWNER PASSWORD REQUIRED.");
  if(!passwordOk)return;
  if(currentProfile?.role!=="owner")return;
...
window.permanentDeleteAllCheckTips=async function(){
  const passwordOk=await requireCurrentAccountPassword("Permanent Delete All","OWNER PASSWORD REQUIRED.");
  if(!passwordOk)return;
  if(currentProfile?.role!=="owner")return;
...
function pdfReportContent(r,index,total){
  ...
  const payoutVals=[
    ...
    ["Adjustment",r.adjustmentDecision==="NONE"?"NO ADJUSTMENT":String(r.adjustmentDecision||"-")]
  ];
  for(let i=0;i<3;i++){
  ...
  // Formula note
  c+=txt("F1",8.5,28, bartender?110:198, "Total Paid Out = Total Before Meal - Meal   |   Grand Total = Total Before Meal + Cash Tip");
...
window.sendSmallReportSms=async function(reportId){
  ...
  const msg=`Hi ${r.employee||""}, your complete signed tip report for ${r.date||""} (${r.shift||""}) is ready. Total Paid Out: ${fmtMoney(smallReportPaidOut(r))}. Cash Tip: ${fmtMoney(r.cashTip)}. Grand Total: ${fmtMoney(smallReportGrandTotal(r))}. The complete PDF has been downloaded on this device; attach that PDF to this message.`;
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.17K hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.17K Busser split. Preserve the existing V01 total busser formula,
  ...
  // V13.8.17K payout rule: employee payout is Total Before Meal minus Meal.
    ...
    body=`<h3 class="how-title">Employee Report — Quick Edit</h3>
      ...
      <div class="how-card">
        ...
        <div class="how-quick-edit-grid">
          ${howField("Grand Total ($)","howGrand",howVal("hGrandTotal"),"number","0.00")}
          ${quickShowAM?howField("Total AM ($)","howTotalAM",howVal("hTotalAM"),"number","0.00"):""}
          ${howField("Paid Tip ($)","howPaid",howVal("hPaidTip"),"number","0.00")}
          ${howField("Pay Card Tip Fee ($)","howCardFee",howVal("hCardFee"),"number","0.00")}
          ${howField("Cash Tip ($)","howCash",howVal("hCashTip"),"number","0.00")}
          ${howField("Meal ($)","howMeal",howVal("hMeal"),"number","0.00")}
        ...
        ${hourlyWizardState.position==="Bartender"
          ? (()=>{const bt=howBartenderFormula();return `<div class="how-summary" style="margin-top:14px">
...
// V13.8.17K Calculate V01 + fresh page fix.

// V13.8.17K: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.17K Employee Clear All fix.

// V13.8.17K Daily Report tab + signature XLS.

// V13.8.17K dedicated Hourly Adjustment sub-app.

// V13.8.17K fresh page generated from current index.

// V13.8.17K Edit finalized Hourly report from Daily Report.

// V13.8.17K Daily Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.17K staff Remember Me with Firebase local/session persistence.

// V13.8.17K Main App button removed from Hourly workspace.

// V13.8.17K Daily Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.17K Remember Me for dedicated Hourly login.

// V13.8.17K Daily Report server signature + SMS.

// V13.8.17K Employee phone directory for Daily Report SMS.

// V13.8.17K recovered employee phone directory + reliable SMS composer.

// V13.8.17K Add/Edit employee + phone directory.

// V13.8.17K Tip Calculation batch workspace.

// V13.8.17K login role selector responsive UI fix.

// V13.8.17K Delete Team.

// V13.8.17K Delete individual Tip Calculation team member.

// V13.8.17K Cloud-backed Tip Calculation drafts.

// V13.8.17K live bar calculation after partial Grand Total save.

// V13.8.17K Team BAR Center with AM / 2-4 / PM automation.

// V13.8.17K show password checkboxes.

// V13.8.17K reliable team-card and BAR server input interactions.

// V13.8.17K separate Team Board and employee editor views.

// V13.8.17K persist BAR state to hourlyV1Batches cloud document.

// V13.8.17K secure Clear All, Owner Undo and permanent deletion.

// V13.8.17K BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.17K: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.17K: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.17K Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.17K employee soft-delete ownership uses Firebase UID.

// V13.8.17K finalized edit hydration, autosave, quick Team Board/BAR navigation.

// V13.8.17K: direct final editing and two-way BAR/server sync.

// V13.8.17K larger UI, complete signed PDF/SMS, Daily Report Grand Total.

// V13.8.17K Grand Total = Total Before Meal + Cash Tip.

// V13.8.17K larger Daily Report; frozen Date + Employee columns.

// V13.8.17K simple Daily Report list, detail modal, Busser AM/PM split.

// V13.8.17K Daily Report stays in Tip Calculation; employee detail is a true modal; history retained.

// V13.8.17K Daily Report always visible in Tip Calculation.

// V13.8.17K restores full management tabs when logging in through Hourly V01.

// V13.8.17K professional non-overlapping PDF employee report layout.

// V13.8.17K removes Hourly V01 login and renames Hourly V1 to Tip Calculation.

// V13.8.17K clean role login buttons and simplified staff dashboard.

// V13.8.17K responsive all-device layout; owner credential UI does not expose plaintext Firebase passwords.

// V13.8.17K cashier role/UI removed.

// V13.8.17K stock-style Analytics chart page.

// V13.8.17K password-protected per-person delete + finalized report restore.

// V13.8.17K Future UI styling / mobile readability.

// V13.8.17K password-gated deletes + Owner Deleted/Undo recovery.

// V13.8.17K successful-login welcome voice + original WebAudio music sting.
