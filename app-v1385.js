...
// V13.8.5 — reusable Show Password checkbox for all password/PIN inputs.
...
function hv1ApplyBarAutomation(s){
  ...
  for(const name of s.team||[]){
    ...
    const amFee=amGT>0?amGT*0.006:0;
    const fee24=gt24>0?(gt24*0.006-amFee):0;
    const pmFee=pmGT>0?(pmGT*0.006-amFee-fee24):0;
    const totalFee=amFee+fee24+pmFee;
...
// V13.8.5 reliable team-card click handling (desktop/tablet/mobile).
...
window.sendSmallReportSms=async function(reportId){
  ...
  const msg=`Hi ${r.employee||""}, your detailed signed tip report for ${r.date||""} (${r.shift||""}) is attached. Total Paid Out: ${fmtMoney(smallReportPaidOut(r))}.`;

  if(navigator.share && navigator.canShare && navigator.canShare({files:[file]})){
    try{
      await navigator.share({
        files:[file],
        title:`Tip Report - ${r.employee||""}`,
        text:`Send this signed PDF to ${r.employee||""} (${phone}) using Messages/SMS. ${msg}`
      });
      return;
    }catch(e){
      if(e.name==="AbortError")return;
      console.warn("PDF/SMS file share:",e);
    }
  }

  downloadBlob(blob,filename);
  await openNativeSmsComposer(phone,msg+" Attach the downloaded PDF to this message.");
};
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.5 hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.5 payout rule: employee payout is Total Before Meal minus Meal.
...
function renderHourlyWizard(){
  ...
  if(hourlyWizardStep===6){
    if(hourlyWizardState.position==="Bartender"){
      ...
      const amTip=barAuto?Number(barAuto.amFee||0):(howVal("hAmBar")==="yes"?totalAM*0.006:0);
      const tip24=barAuto?Number(barAuto.fee24||0):0;
      const pmTip=barAuto?Number(barAuto.pmFee||0):(howVal("hPmBar")==="yes"?totalPM*0.006:0);
      const totalBarTip=barAuto?Number(barAuto.totalFee||0):(amTip+pmTip);
      const amChecked=barAuto?Number(barAuto.amGT||0)>0:howVal("hAmBar")==="yes";
      const pmChecked=barAuto?(Number(barAuto.gt24||0)>0||Number(barAuto.pmGT||0)>0):howVal("hPmBar")==="yes";

      body=`<h3 class="how-title">Bar Tip Out</h3><p class="how-sub">${barAuto?"BAR Center is the source of truth. These values are automatic.":"Check only the shift that had bar sales."}</p>
        <div class="how-card">
          ${am?`<label class="check-card"><input id="howAmBar" type="checkbox" ${amChecked?"checked":""} ${barAuto?"disabled":""}><span><strong>AM BAR SALES</strong><span>${barAuto?`Cumulative ${howMoney(barAuto.amGT||0)}`:"Checked = 0.6% of Total AM"}</span></span></label>`:""}
          ${pm?`<label class="check-card"><input id="howPmBar" type="checkbox" ${pmChecked?"checked":""} ${barAuto?"disabled":""}><span><strong>2–4 / PM BAR SALES</strong><span>${barAuto?`2–4 cumulative ${howMoney(barAuto.gt24||0)} • PM cumulative ${howMoney(barAuto.pmGT||0)}`:"Checked = 0.6% of Total PM"}</span></span></label>`:""}
          <div class="how-metrics">
...
// V13.8.5 Calculate V01 + fresh page fix.

// V13.8.5: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.5 Employee Clear All fix.

// V13.8.5 Small Report tab + signature XLS.

// V13.8.5 dedicated Hourly Adjustment sub-app.

// V13.8.5 fresh page generated from current index.

// V13.8.5 Edit finalized Hourly report from Small Report.

// V13.8.5 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.5 staff Remember Me with Firebase local/session persistence.

// V13.8.5 Main App button removed from Hourly workspace.

// V13.8.5 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.5 Remember Me for dedicated Hourly login.

// V13.8.5 Small Report server signature + SMS.

// V13.8.5 Employee phone directory for Small Report SMS.

// V13.8.5 recovered employee phone directory + reliable SMS composer.

// V13.8.5 Add/Edit employee + phone directory.

// V13.8.5 Hourly Adjustment V1 batch workspace.

// V13.8.5 login role selector responsive UI fix.

// V13.8.5 Delete Team.

// V13.8.5 Delete individual Hourly V1 team member.

// V13.8.5 Cloud-backed Hourly V1 drafts.

// V13.8.5 live bar calculation after partial Grand Total save.

// V13.8.5 Team BAR Center with AM / 2-4 / PM automation.

// V13.8.5 show password checkboxes.

// V13.8.5 reliable team-card and BAR server input interactions.

// V13.8.5 separate Team Board and employee editor views.

// V13.8.5 persist BAR state to hourlyV1Batches cloud document.

// V13.8.5 secure Clear All, Owner Undo and permanent deletion.

// V13.8.5 BAR source-of-truth + payout report + signed PDF/SMS.
