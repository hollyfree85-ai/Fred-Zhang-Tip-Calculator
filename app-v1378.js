...
// V13.7.8 — reusable Show Password checkbox for all password/PIN inputs.
...
function hv1RenderBarSummaryOnly(state=hv1Load()){
  ...
  host.innerHTML=`<h4>Server Bar Tip Out Summary</h4>
    ...
    ${allServers.map(name=>{
      const am=(Number(state.bar.AM.entries?.[name])||0)*0.006;
      const p24=(Number(state.bar["2PM_4PM"].entries?.[name])||0)*0.006;
      const pm=(Number(state.bar.PM.entries?.[name])||0)*0.006;
...
// V13.7.8 reliable team-card click handling (desktop/tablet/mobile).
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.7.8 hard guard: visible Step-6 result stays authoritative.
...
// V13.7.8 Calculate V01 + fresh page fix.

// V13.7.8: Step-7 no longer erases Bartender Step-6 server totals.

// V13.7.8 Employee Clear All fix.

// V13.7.8 Small Report tab + signature XLS.

// V13.7.8 dedicated Hourly Adjustment sub-app.

// V13.7.8 fresh page generated from current index.

// V13.7.8 Edit finalized Hourly report from Small Report.

// V13.7.8 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.7.8 staff Remember Me with Firebase local/session persistence.

// V13.7.8 Main App button removed from Hourly workspace.

// V13.7.8 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.7.8 Remember Me for dedicated Hourly login.

// V13.7.8 Small Report server signature + SMS.

// V13.7.8 Employee phone directory for Small Report SMS.

// V13.7.8 recovered employee phone directory + reliable SMS composer.

// V13.7.8 Add/Edit employee + phone directory.

// V13.7.8 Hourly Adjustment V1 batch workspace.

// V13.7.8 login role selector responsive UI fix.

// V13.7.8 Delete Team.

// V13.7.8 Delete individual Hourly V1 team member.

// V13.7.8 Cloud-backed Hourly V1 drafts.

// V13.7.8 live bar calculation after partial Grand Total save.

// V13.7.8 Team BAR Center with AM / 2-4 / PM automation.

// V13.7.8 show password checkboxes.

// V13.7.8 reliable team-card and BAR server input interactions.
