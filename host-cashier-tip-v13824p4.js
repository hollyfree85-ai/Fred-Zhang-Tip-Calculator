...
let state={date:today(),cashAM:0,creditAM:0,cashPM:0,creditPM:0,team:{},employeesAM:Array(7).fill(""),employeesPM:Array(7).fill(""),signatures:{AM:{},PM:{}}};
let unsub=null,dirty=false,sigCtx=null,sigTarget=null,sigStrokes=[],drawing=false,currentStroke=null;

function roleAllowed(){return ["manager","owner"].includes(String(window.__getCurrentRole?.()||""))}
...
function renderShiftRows(shift){
  ...
  host.innerHTML=Array.from({length:7},(_,i)=>{
    ...
    return `<div class="hc-row"><span class="hc-row-num">${i+1}</span>
      ...
      <button class="btn light hc-sign-btn ${signed?"hc-signed":""}" type="button" data-index="${i}">${signed?"✓ Signed":"Sign"}</button></div>`;
  }).join("");
  ...
  host.querySelectorAll(".hc-sign-btn").forEach(btn=>btn.addEventListener("click",()=>{
    const idx=Number(btn.dataset.index),name=state[key][idx];if(!name){alert("Select an employee first.");return}openSignature(shift,name);
  }));
}
...
function calc(){
  ...
  const am=namesFor("AM"),pm=namesFor("PM"),eachAM=am.length?poolAM/am.length:0,eachPM=pm.length?poolPM/pm.length:0;
  $("hcPoolAM").textContent=money(poolAM);$("hcPoolPM").textContent=money(poolPM);
  $("hcCountAM").textContent=am.length;$("hcCountPM").textContent=pm.length;
  $("hcEachAM").textContent=money(eachAM);$("hcEachPM").textContent=money(eachPM);
  for(let i=0;i<7;i++){const a=$(`hcTipAM${i}`),p=$(`hcTipPM${i}`);if(a)a.textContent=state.employeesAM[i]?money(eachAM):money(0);if(p)p.textContent=state.employeesPM[i]?money(eachPM):money(0)}
  return {poolAM,poolPM,eachAM,eachPM,countAM:am.length,countPM:pm.length};
}
function bindMoney(){
  [["hcCashAM","cashAM"],["hcCreditAM","creditAM"],["hcCashPM","cashPM"],["hcCreditPM","creditPM"]].forEach(([id,key])=>{
    const el=$(id);el.value=state[key]||0;el.oninput=()=>{state[key]=num(el.value);dirty=true;calc()};
  });
}
function applyState(data){
  state={date:data?.date||state.date||today(),cashAM:num(data?.cashAM),creditAM:num(data?.creditAM),cashPM:num(data?.cashPM),creditPM:num(data?.creditPM),team:data?.team||{},employeesAM:Array.isArray(data?.employeesAM)?[...data.employeesAM,...Array(7).fill("")].slice(0,7):Array(7).fill(""),employeesPM:Array.isArray(data?.employeesPM)?[...data.employeesPM,...Array(7).fill("")].slice(0,7):Array(7).fill(""),signatures:data?.signatures||{AM:{},PM:{}},savedBy:data?.savedBy||""};
...
async function save(){
  ...
  const payload={...state,date:$("hcDate").value||today(),cashAM:num($("hcCashAM").value),creditAM:num($("hcCreditAM").value),cashPM:num($("hcCashPM").value),creditPM:num($("hcCreditPM").value),metrics,savedBy:profile.displayName||profile.role||"Staff",savedByUid:auth.currentUser?.uid||"",updatedAt:serverTimestamp()};
...
function setupCanvas(){
  const c=$("hcSignatureCanvas");sigCtx=c.getContext("2d");sigCtx.lineWidth=3;sigCtx.lineCap="round";sigCtx.strokeStyle="#102b43";
...
function openSignature(shift,name){sigTarget={shift,name};sigStrokes=JSON.parse(JSON.stringify(state.signatures?.[shift]?.[name]?.strokes||[]));$("hcSignatureTitle").textContent=`${name} — ${shift} Signature`;$("hcSignatureModal").classList.remove("hidden");setTimeout(redrawSig,30)}
function closeSignature(){$("hcSignatureModal").classList.add("hidden");sigTarget=null}
...
function init(){
  const tab=document.querySelector('[data-stab="hostCashierTip"]');if(!tab)return;
  $("hcDate").value=state.date;renderRoster();bindMoney();renderRows();setupCanvas();
  tab.addEventListener("click",()=>setTimeout(()=>{if(roleAllowed())loadDate()},40));
