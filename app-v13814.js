...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    alert(`Delete failed: ${e.code||e.message}\n\nIf this says permission-denied, publish the V13.8.14 Firestore rules.`);
  }
};
...
window.renderSmallReport=function(){
  ...
  body.innerHTML=rows.map(r=>{
    ...
    return `<tr>
      <td>${esc(r.date||"")}</td>
      <td class="sr-freeze-employee"><b>${esc(r.employee||"")}</b><div class="small">${esc(r.position||"")}</div></td>
      <td>${esc(r.shift||"")}</td>
      <td>${esc(c.in1||"-")}</td>
      <td>${esc(c.out1||"-")}</td>
      <td>${esc(c.in2||"-")}</td>
...
// V13.8.14 finalized edit hydration, autosave, quick Team Board/BAR navigation.

// V13.8.14: direct final editing and two-way BAR/server sync.

// V13.8.14 larger UI, complete signed PDF/SMS, Small Report Grand Total.

// V13.8.14 Grand Total = Total Before Meal + Cash Tip.

// V13.8.14 larger Small Report; frozen Date + Employee columns.
