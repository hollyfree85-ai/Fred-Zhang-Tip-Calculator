...
// V13.8.14 — reusable Show Password checkbox for all password/PIN inputs.
...
// V13.8.14 reliable team-card click handling (desktop/tablet/mobile).
...
function showApp(){
  ...
  if(emp){
    // Employee portal is read-only now: finalized My Tip Reports only.
    $("employeeBottom")?.classList.add("hidden");
...
function listenEmployeeHistoricalReports(){
  ...
  unsubs.push(onSnapshot(qh,snap=>{
    ...
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.14 hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.14 Firestore rules.</div>`;
    }
  }
}
async function archiveCheckTipSheet(sheet){
  ...
  // and that caused the permission-denied seen in V13.8.14.
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    console.error("Employee Check Tip archive V13.8.14:",e);
    alert(`Delete archive failed: ${e.code||e.message}\n\nPublish the V13.8.14 Firestore rules if this says permission-denied.`);
    return;
  }
  ...
  try{
    ...
    console.warn("Post-delete refresh V13.8.14:",e);
  }
};
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.14 hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.14 payout rule: employee payout is Total Before Meal minus Meal.
...
// V13.8.14 Calculate V01 + fresh page fix.

// V13.8.14: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.14 Employee Clear All fix.

// V13.8.14 Small Report tab + signature XLS.

// V13.8.14 dedicated Hourly Adjustment sub-app.

// V13.8.14 fresh page generated from current index.

// V13.8.14 Edit finalized Hourly report from Small Report.

// V13.8.14 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.14 staff Remember Me with Firebase local/session persistence.

// V13.8.14 Main App button removed from Hourly workspace.

// V13.8.14 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.14 Remember Me for dedicated Hourly login.

// V13.8.14 Small Report server signature + SMS.

// V13.8.14 Employee phone directory for Small Report SMS.

// V13.8.14 recovered employee phone directory + reliable SMS composer.

// V13.8.14 Add/Edit employee + phone directory.

// V13.8.14 Hourly Adjustment V1 batch workspace.

// V13.8.14 login role selector responsive UI fix.

// V13.8.14 Delete Team.

// V13.8.14 Delete individual Hourly V1 team member.

// V13.8.14 Cloud-backed Hourly V1 drafts.

// V13.8.14 live bar calculation after partial Grand Total save.

// V13.8.14 Team BAR Center with AM / 2-4 / PM automation.

// V13.8.14 show password checkboxes.

// V13.8.14 reliable team-card and BAR server input interactions.

// V13.8.14 separate Team Board and employee editor views.

// V13.8.14 persist BAR state to hourlyV1Batches cloud document.

// V13.8.14 secure Clear All, Owner Undo and permanent deletion.

// V13.8.14 BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.14: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.14: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.14 Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.14 employee soft-delete ownership uses Firebase UID.

// V13.8.14 employee Check Tip self-delete rule compatibility fix.

// V13.8.14 employee soft-delete skips forbidden pre-read of missing archive doc.

// V13.8.14 employee deletedItems query matches Firestore read-rule ownership.

// V13.8.14 Employee Check Tip removed; simple Cashier payout table added.

// V13.8.14 Employee portal is finalized My Tip Reports only.
