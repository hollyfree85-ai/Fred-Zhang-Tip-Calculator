...
// V13.8.27 — reusable Show Password checkbox for all password/PIN inputs.
...
async function requireCurrentAccountPassword(title,text,allowedRoles=["manager","owner"]){
  ...
  // Owner biometric is optional and ONLY attempted after login.
  if(role==="owner" && fzOwnerBioRec()){
    const ok=await fzOwnerBioVerify();
    if(ok)return true;
  }
...
// V13.8.27 reliable team-card click handling (desktop/tablet/mobile).
...
window.openHourlyWorkspacePanel=function(name){
  // V13.8.27A: Hourly V01 is no longer an isolated sub-app.
...
onAuthStateChanged(auth, async user=>{
  ...
  try{
    ...
    showApp();
    // V13.8.27: every successful authenticated login gets a short music sting + voice welcome.
...
function listenEmployeeHistoricalReports(){
  ...
  const fail=e=>{
    ...
    if(host && !window.__employeeHistoricalHourlyReports?.length){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.27 hourlyReports rule and run Sync Historical Reports once as Manager/Owner.</span></div>`;
    }
  };
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.27 Firestore rules.</div>`;
    }
  }
}
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    alert(`Delete failed: ${e.code||e.message}\n\nIf this says permission-denied, publish the V13.8.27 Firestore rules.`);
  }
};
...
window.deleteAppUser=async function(uid){
  ...
  if(target.role==="owner"){alert("Owner account cannot be deleted here.");return;}
  ...
  const ok=await requireCurrentAccountPassword(
    ...
    "OWNER CONFIRMATION REQUIRED. This permanently removes the login account and user profile. Historical tip reports are kept for future re-linking."
  );
  if(!ok)return;
  if(!confirm(`Permanently delete ${name}? This account cannot be restored with Undo. Historical tip reports will remain.`))return;

  try{
    // Existing Admin callable should delete Firebase Authentication user.
    ...
    // Remove Firestore account records.
    try{const s=await getDoc(doc(db,"users",uid));if(s.exists())await deleteDoc(doc(db,"users",uid));}catch(e){console.warn(e)}
    try{const s=await getDoc(doc(db,"signupRequests",uid));if(s.exists())await deleteDoc(doc(db,"signupRequests",uid));}catch(e){console.warn(e)}

    // Remove only old deleted-user archive entries; keep historical tip reports.
    try{
      const ds=await getDocs(query(collection(db,"deletedItems"),where("employeeUid","==",uid),limit(100)));
      for(const d of ds.docs){
        if((d.data()||{}).itemType==="user_profile")await deleteDoc(d.ref);
      }
    }catch(e){console.warn(e)}

    alert(`${name} was permanently deleted.`);
  }catch(e){
    console.error("Permanent delete:",e);
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.27 hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.27 Busser split. Preserve the existing V01 total busser formula,
  ...
  // V13.8.27 authoritative minimum-hourly adjustment rule.
...
// V13.8.27 Calculate + fresh page fix.

// V13.8.27: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.27 Employee Clear All fix.

// V13.8.27 Daily Report tab + signature XLS.

// V13.8.27 dedicated Hourly Adjustment sub-app.

// V13.8.27 fresh page generated from current index.

// V13.8.27 Edit finalized Hourly report from Daily Report.

// V13.8.27 Daily Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.27 staff Remember Me with Firebase local/session persistence.

// V13.8.27 Main App button removed from Hourly workspace.

// V13.8.27 Daily Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.27 Remember Me for dedicated Hourly login.

// V13.8.27 Daily Report server signature + SMS.

// V13.8.27 Employee phone directory for Daily Report SMS.

// V13.8.27 recovered employee phone directory + reliable SMS composer.

// V13.8.27 Add/Edit employee + phone directory.

// V13.8.27 Tip Calculation batch workspace.

// V13.8.27 login role selector responsive UI fix.

// V13.8.27 Delete Team.

// V13.8.27 Delete individual Tip Calculation team member.

// V13.8.27 Cloud-backed Tip Calculation drafts.

// V13.8.27 live bar calculation after partial Grand Total save.

// V13.8.27 Team BAR Center with AM / 2-4 / PM automation.

// V13.8.27 show password checkboxes.

// V13.8.27 reliable team-card and BAR server input interactions.

// V13.8.27 separate Team Board and employee editor views.

// V13.8.27 persist BAR state to hourlyV1Batches cloud document.

// V13.8.27 secure Clear All, Owner Undo and permanent deletion.

// V13.8.27 BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.27: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.27: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.27 Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.27 employee soft-delete ownership uses Firebase UID.

// V13.8.27 finalized edit hydration, autosave, quick Team Board/BAR navigation.

// V13.8.27: direct final editing and two-way BAR/server sync.

// V13.8.27 larger UI, complete signed PDF/SMS, Daily Report Grand Total.

// V13.8.27 Grand Total = Total Before Meal + Cash Tip.

// V13.8.27 larger Daily Report; frozen Date + Employee columns.

// V13.8.27 simple Daily Report list, detail modal, Busser AM/PM split.

// V13.8.27 Daily Report stays in Tip Calculation; employee detail is a true modal; history retained.

// V13.8.27 Daily Report always visible in Tip Calculation.

// V13.8.27 restores full management tabs when logging in through Hourly V01.

// V13.8.27 professional non-overlapping PDF employee report layout.

// V13.8.27 removes Hourly V01 login and renames Hourly V1 to Tip Calculation.

// V13.8.27 clean role login buttons and simplified staff dashboard.

// V13.8.27 responsive all-device layout; owner credential UI does not expose plaintext Firebase passwords.

// V13.8.27 cashier role/UI removed.

// V13.8.27 stock-style Analytics chart page.

// V13.8.27 password-protected per-person delete + finalized report restore.

// V13.8.27 Future UI styling / mobile readability.

// V13.8.27 password-gated deletes + Owner Deleted/Undo recovery.

// V13.8.27 successful-login welcome voice + original WebAudio music sting.

// V13.8.27 primes audio/speech during actual login gesture.

setTimeout(()=>{fzOpenGuestReportPortalFromUrl().catch(()=>{});},50);
// V13.8.27: Employee report-only experience + private no-signup portal.

// V13.8.27 adjustment examples:
...
// V13.8.27 Owner biometric is intentionally isolated from login/auth.
// It is used only AFTER a successful Owner login for sensitive confirmations.
const FZ_OWNER_BIO_KEY="fz_owner_bio_v13827";
let fzOwnerBioVerifiedAt=0;

function fzOwnerBioSupported(){
  return !!(window.PublicKeyCredential && navigator.credentials && window.isSecureContext);
}
function fzOwnerBioRec(){
  try{return JSON.parse(localStorage.getItem(FZ_OWNER_BIO_KEY)||"null")}catch(e){return null}
}
function fzRand(n=32){const a=new Uint8Array(n);crypto.getRandomValues(a);return a}
function fzB64u(a){
  let s="";new Uint8Array(a).forEach(b=>s+=String.fromCharCode(b));
  return btoa(s).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/,"");
}
function fzUnb64u(s){
  s=String(s).replace(/-/g,"+").replace(/_/g,"/");while(s.length%4)s+="=";
  const r=atob(s),a=new Uint8Array(r.length);for(let i=0;i<r.length;i++)a[i]=r.charCodeAt(i);return a;
}
async function fzOwnerBioVerify(){
  const rec=fzOwnerBioRec();
  if(!rec||!fzOwnerBioSupported())return false;
  if(String(rec.uid||"")!==String(currentUser?.uid||""))return false;
  if(Date.now()-fzOwnerBioVerifiedAt<15000)return true;
  try{
    const x=await navigator.credentials.get({publicKey:{
      challenge:fzRand(),userVerification:"required",timeout:60000,
      allowCredentials:[{type:"public-key",id:fzUnb64u(rec.id),transports:["internal"]}]
    }});
    if(!x)return false;
    fzOwnerBioVerifiedAt=Date.now();
    return true;
  }catch(e){return false}
}
window.toggleOwnerBiometric=async function(){
  if(currentProfile?.role!=="owner"){alert("Owner only.");return}
  if(!fzOwnerBioSupported()){alert("Biometric/passkey is not supported on this device/browser.");return}
  const old=fzOwnerBioRec();
  if(old){
    if(!await fzOwnerBioVerify()){alert("Biometric verification failed.");return}
    localStorage.removeItem(FZ_OWNER_BIO_KEY);fzOwnerBioVerifiedAt=0;
    alert("Owner biometric disabled on this device.");return;
  }
  try{
    const raw=new TextEncoder().encode(String(currentUser?.uid||"owner"));
    const cred=await navigator.credentials.create({publicKey:{
      challenge:fzRand(),
      rp:{name:"Fred Zhang Tip Calculator"},
      user:{id:raw.length>64?raw.slice(0,64):raw,name:String(auth.currentUser?.email||"owner"),displayName:String(currentProfile?.displayName||"Owner")},
      pubKeyCredParams:[{type:"public-key",alg:-7},{type:"public-key",alg:-257}],
      authenticatorSelection:{authenticatorAttachment:"platform",residentKey:"preferred",userVerification:"required"},
      timeout:60000,attestation:"none"
    }});
    if(!cred)throw new Error("No credential");
    localStorage.setItem(FZ_OWNER_BIO_KEY,JSON.stringify({id:fzB64u(cred.rawId),uid:String(currentUser?.uid||""),createdAt:Date.now()}));
    fzOwnerBioVerifiedAt=Date.now();
    alert("Owner biometric enabled on this device.");
  }catch(e){alert(`Biometric setup failed: ${e.name||e.message}`)}
};
