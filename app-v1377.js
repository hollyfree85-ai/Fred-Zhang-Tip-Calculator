...
// V13.7.7 — reusable Show Password checkbox for all password/PIN inputs.
...
async function hv1CloudSave(s){
  ...
  try{
    ...
    hv1SetCloudStatus("Draft backup: SAVED TO CLOUD","ok");
    return true;
  }catch(e){
...
async function hv1CloudRestore(){
  ...
  try{
    ...
    const merged={team:Array.isArray(cloud.team)?cloud.team:[],drafts:{...(cloud.drafts||{})}};
    ...
    hv1SetCloudStatus("Draft backup: RESTORED / SYNCED","ok");
    return merged;
  }catch(e){
...
function hv1RenderCards(){const host=$('hv1Cards'),s=hv1Load();if(!host)return;host.innerHTML=(s.team||[]).map(n=>{const d=s.drafts?.[n]||hv1BlankDraft(n),st=hv1Status(d),cls=st==='COMPLETED'?'final':st==='IN PROGRESS'?'progress':'';const entered=Object.keys(d.entered||{}).length;return `<div class="hv1-card-wrap"><button class="hv1-card ${cls}" onclick="hv1OpenEmployee('${encodeURIComponent(n)}')"><span class="hv1-status">${st}</span><h4>${esc(n)}</h4><div class="hv1-meta">${d.values?.hShift?`Shift: ${esc(d.values.hShift)}`:'Shift not selected'}<br>${entered} field/group(s) saved${(d.skippedPages||[]).length?` • ${(d.skippedPages||[]).length} page(s) skipped`:''}</div></button><button class="hv1-card-delete" type="button" title="Delete ${esc(n)} from this team" onclick="event.stopPropagation();hv1DeleteEmployee('${encodeURIComponent(n)}')">×</button></div>`}).join('')}
async function hv1Enter(){hourlyV1Mode=true;document.body.classList.add('hourly-v1-mode');document.body.classList.remove('hourly-workspace-mode');document.querySelectorAll('.staffPanel').forEach(x=>x.classList.add('hidden'));$('hourlyV1Workspace')?.classList.remove('hidden');if($('hv1Date')&&!$('hv1Date').value)$('hv1Date').value=todayLocal();await hv1CloudRestore();hv1RenderRoster();const s=hv1Load();$('hv1Setup')?.classList.toggle('hidden',!!s.team?.length);$('hv1BoardBox')?.classList.toggle('hidden',!s.team?.length);if(s.team?.length)hv1RenderCards()}
...
function hv1BarCardHtml(key,s){
  ...
  return `<h4>${hv1BarCheckpointLabel(key)}</h4>
    ...
    ${names.length?names.map(name=>{
      ...
      return `<div class="hv1-bar-row"><b>${esc(name)}</b><input type="number" inputmode="decimal" min="0" step="0.01" placeholder="0.00" data-hv1-bar-gt="${key}" data-employee="${encodeURIComponent(name)}" value="${v===''?'':esc(v)}"><div class="hv1-bar-fee">${fmtMoney((Number(v)||0)*0.006)}</div></div>`;
    }).join(''):'<div class="notice">No server with this shift is set yet.</div>'}
    <div class="hv1-bar-metrics">
      <div class="hv1-bar-metric"><span>Gross 0.6%</span><b>${fmtMoney(gross)}</b></div>
      <div class="hv1-bar-metric"><span>Server Count</span><b>${names.length}</b></div>
      <div class="hv1-bar-metric hv1-bar-received"><span>BAR TIP OUT RECEIVED</span><b>${fmtMoney(received)}</b></div>
    </div>`;
}
function hv1RenderBarCenter(){
  ...
  const allServers=[...new Set(['AM','2PM_4PM','PM'].flatMap(k=>hv1ServerNamesForCheckpoint(k,s)))].sort((a,b)=>a.localeCompare(b));
  if($('hv1BarServerTotals')){
    $('hv1BarServerTotals').innerHTML=`<h4>Server Bar Tip Out Summary</h4>
      ...
      ${allServers.map(name=>{
        const am=(Number(s.bar.AM.entries?.[name])||0)*0.006;
        const p24=(Number(s.bar['2PM_4PM'].entries?.[name])||0)*0.006;
        const pm=(Number(s.bar.PM.entries?.[name])||0)*0.006;
  ...
  document.querySelectorAll('[data-hv1-bar-gt]').forEach(inp=>inp.addEventListener('input',()=>{
    ...
    state.bar[key].entries[name]=String(inp.value||'');
...
function hv1ApplyBarAutomation(s){
  ...
  for(const name of s.team||[]){
    ...
    const amFee=amGT*0.006,fee24=gt24*0.006,pmFee=pmGT*0.006,totalFee=amFee+fee24+pmFee;
...
window.hv1BackFromBar=function(){
  ...
  $('hv1BarBox')?.classList.add('hidden');$('hv1BoardBox')?.classList.remove('hidden');hv1RenderCards();
};
window.hv1SaveBarCenter=function(){
  const s=hv1Load();hv1ApplyBarAutomation(s);hv1Save(s);hv1RenderBarCenter();
  alert('BAR calculations saved and pushed to server/bartender cards.');
};
...
window.hv1DeleteTeam=async function(){
  const date=$('hv1Date')?.value||todayLocal();
  const state=hv1Load();
  const count=(state.team||[]).length;
  if(!count){alert('No team is saved for this date.');return;}

  const ok=confirm(
    `Delete the entire Hourly V1 team for ${date}?\n\n`+
    `This removes ${count} employee card(s) and their unfinished V1 draft/progress for this date.\n\n`+
  ...
  try{
    ...
    try{await deleteDoc(doc(db,"hourlyV1Batches",hv1CloudDocId(date)));hv1SetCloudStatus("Draft backup: team deleted","ok");}
    catch(e){console.warn("Hourly V1 cloud delete:",e);hv1SetCloudStatus("Team deleted locally; cloud delete blocked","warn");}
    hv1EditingEmployee='';
    $('hv1BoardBox')?.classList.add('hidden');
    $('hv1Setup')?.classList.remove('hidden');
    if($('hv1Cards'))$('hv1Cards').innerHTML='';
    hv1RenderRoster();
    alert(`Team for ${date} deleted.`);
  }catch(e){
    console.error('Hourly V1 Delete Team:',e);
    alert('Delete Team failed: '+(e.message||e));
  }
};
...
window.hv1DateChanged=async function(){$('hv1BarBox')?.classList.add('hidden');await hv1CloudRestore();hv1RenderRoster();const s=hv1Load();$('hv1Setup').classList.toggle('hidden',!!s.team?.length);$('hv1BoardBox').classList.toggle('hidden',!s.team?.length);if(s.team?.length)hv1RenderCards()};
window.hv1OpenEmployee=function(name){try{name=decodeURIComponent(name)}catch(e){}hv1EditingEmployee=name;document.body.classList.add('hourly-v1-editing');$('hourlyV1Workspace').classList.add('hidden');$('hourly')?.classList.remove('hidden');hv1ApplyDraft(name)};
window.hv1BackToBoard=function(){hv1CapturePage(false);document.body.classList.remove('hourly-v1-editing');$('hourly')?.classList.add('hidden');$('hourlyV1Workspace').classList.remove('hidden');hv1RenderCards()};
...
window.hv1OpenSmallReport=function(){document.body.classList.remove('hourly-v1-mode');$('hourlyV1Workspace').classList.add('hidden');$('smallReport')?.classList.remove('hidden');renderSmallReport()};
...
window.logout = async function(){
  hourlyWorkspaceRequested=false;hourlyV1Requested=false;hourlyV1Mode=false;hv1EditingEmployee="";document.body.classList.remove("hourly-v1-mode","hourly-v1-editing");
  applyHourlyWorkspaceMode(false);
  try{
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.7.7 hard guard: visible Step-6 result stays authoritative.
...
// V13.7.7 Calculate V01 + fresh page fix.

// V13.7.7: Step-7 no longer erases Bartender Step-6 server totals.

// V13.7.7 Employee Clear All fix.

// V13.7.7 Small Report tab + signature XLS.

// V13.7.7 dedicated Hourly Adjustment sub-app.

// V13.7.7 fresh page generated from current index.

// V13.7.7 Edit finalized Hourly report from Small Report.

// V13.7.7 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.7.7 staff Remember Me with Firebase local/session persistence.

// V13.7.7 Main App button removed from Hourly workspace.

// V13.7.7 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.7.7 Remember Me for dedicated Hourly login.

// V13.7.7 Small Report server signature + SMS.

// V13.7.7 Employee phone directory for Small Report SMS.

// V13.7.7 recovered employee phone directory + reliable SMS composer.

// V13.7.7 Add/Edit employee + phone directory.

// V13.7.7 Hourly Adjustment V1 batch workspace.

// V13.7.7 login role selector responsive UI fix.

// V13.7.7 Delete Team.

// V13.7.7 Delete individual Hourly V1 team member.

// V13.7.7 Cloud-backed Hourly V1 drafts.

// V13.7.7 live bar calculation after partial Grand Total save.

// V13.7.7 Team BAR Center with AM / 2-4 / PM automation.

// V13.7.7 show password checkboxes.
