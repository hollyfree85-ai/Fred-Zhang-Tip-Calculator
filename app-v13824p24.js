...
function hv1BarReceived(key,s){
  // P24 AUTHORITATIVE BAR RECEIVED:
  // received for a checkpoint = SUM of each server's non-negative THIS-PERIOD fee.
  // This prevents negative BAR 2–4 / PM when a later cumulative checkpoint is blank.
  return hv1ServerNamesForCheckpoint(key,s)
    .reduce((sum,name)=>sum+hv1ServerCheckpointFee(name,key,s),0);
}
...
function hv1BarCardHtml(key,s){
  ...
  return `<h4>${hv1BarCheckpointLabel(key)}</h4>
    ...
    }).join(''):'<div class="notice">No server is available. Add employees to Today\'s Team first.</div>'}
...
function hv1PatchWizard(){
  ...
  if(!body.querySelector('.hv1-wizard-tools')){
    const tools=document.createElement('div');
    tools.className='hv1-wizard-tools';
    tools.innerHTML='<button class="btn light" type="button" onclick="hv1BackToBoard()">TEAM BOARD</button><button class="btn light" type="button" onclick="hv1SkipPage()">SKIP PAGE</button><button class="btn gold" type="button" onclick="hv1SavePage()">SAVE PROGRESS</button>';
    body.prepend(tools);
  }
  if(!body.querySelector('.hv1-back-team-bottom')){
    const bottom=document.createElement('div');
    bottom.className='hv1-back-team-bottom';
    bottom.innerHTML='<button class="btn dark" type="button" onclick="hv1SavePage()">SAVE & BACK TO TEAM BOARD</button>';
...
window.calculateHourlyV01=function(){
  ...
  // P24 AUTHORITATIVE FINAL FORMULA INVARIANTS.
  // These are recalculated AFTER all BAR / bartender overrides so old engine values cannot drift.
  lastHourlyResult.cashTip=Number($("hCashTip")?.value||lastHourlyResult.cashTip||0);
  lastHourlyResult.meal=Number($("hMeal")?.value||lastHourlyResult.meal||0);

  // Grand Total Tip = Total Before Meal + Cash Tip.
  lastHourlyResult.grandTotalTip=
    Number(lastHourlyResult.totalBeforeMeal||0)+Number(lastHourlyResult.cashTip||0);

  // Accepted hourly adjustment is displayed separately in Grand Total After Adjustment.
  lastHourlyResult.grandTotalAfterAdjustment=
    Number(lastHourlyResult.grandTotalTip||0)+Number(lastHourlyResult.adjustmentSalaryHourly||0);

  // Cash Tip is already received by employee and MUST NOT be paid out again.
  // Total Paid Out = Total Before Meal - Meal.
  lastHourlyResult.totalPaidOut=Math.max(
    0,
    Number(lastHourlyResult.totalBeforeMeal||0)-Number(lastHourlyResult.meal||0)
  );
    ...
    body=`<h3 class="how-title">Employee Report</h3><p class="how-sub">Calculate, review the V01 result, then submit final when everything is correct.</p>
      <div class="how-card">
        ...
        <div class="how-review-actions">
          ...
          <button type="button" class="btn dark" onclick="hv1SavePage()">SAVE & BACK TO TEAM BOARD</button>
        </div>
...
// V13.8.24-P22: TEAM CLOUD-SAVE WAIT + MANUAL BAR SERVER ENTRY RESTORED. Formula logic unchanged.

// V13.8.24-P23: CASH TIP EXCLUDED FROM TOTAL PAID OUT + SAVE/BACK TEAM BOARD BUTTON.

window.__FZ_FORMULA_AUDIT_V13824P24=Object.freeze({
  paidTip:"Used directly; busser/card fee are not deducted a second time",
  serverBeforeMeal:"Paid Tip - Server Bar Tip Out",
  bartenderBeforeMeal:"Paid Tip + Bartender Bar Tip Out Received",
  cashTip:"Separate; excluded from Total Paid Out",
  grandTotalTip:"Total Before Meal + Cash Tip",
  totalPaidOut:"Total Before Meal - Meal",
  serverBarAM:"AM cumulative Grand Total × 0.6%",
  serverBar24:"max(0, 2–4 cumulative × 0.6% - AM fee)",
  serverBarPM:"max(0, PM cumulative × 0.6% - AM fee - 2–4 fee)",
  bartenderReceived:"Sum of all servers' THIS-PERIOD fees for that checkpoint",
  hourlyMinimumServer:"Hours × $7.25",
  hourlyMinimumBartender:"Hours × $7.00"
});

// V13.8.24-P24: FULL FORMULA AUDIT + BAR RECEIVED NON-NEGATIVE SUM FIX.
