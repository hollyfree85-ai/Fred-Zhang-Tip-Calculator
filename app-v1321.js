...
function renderEmployeeTipCheckStatus(){
  ...
  const rows=latestTipCheckSheets.slice(0,10);
  el.innerHTML=rows.length?rows.map(r=>{
    const issues=tipIssueCount(r);
    return `<div class="tip-check-sheet">
      <div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap">
        <b>${esc(r.date||"")}</b>
        <span class="status ${r.status==="cashier_completed"?"approved":"pending"}">${tipCheckStatusLabel(r.status)}</span>
      </div>
      <div class="small">${Number(r.rowCount||r.rows?.length||0)} line(s) • Total Tip ${fmtMoney(r.totalTip||0)}${r.status==="cashier_completed"?` • ${issues} issue(s)`:""}</div>
      ${r.status==="cashier_completed"?`
        <div class="notice ${issues?"warning":"good"}" style="margin-top:8px">
          <b>Cashier has uploaded your tips into the system. Please check.</b>
          ${issues?`<div style="margin-top:4px">Problem lines are marked below.</div>`:"<div style='margin-top:4px'>All lines were completed successfully.</div>"}
        </div>
...
function cashierSheetHtml(r){
  ...
  return `<div class="tip-check-sheet" data-cashier-sheet="${r.id}">
    ...
    <div class="notice" style="margin-top:10px">Every row must have one result: <b>Done</b>, <b>No Signature</b>, or <b>Ticket Not Found</b>.</div>
