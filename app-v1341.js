...
function showApp(){
  ...
  // Employee build: Check Tip only. Manager/Owner staff workflows remain unchanged.
  if(emp){
    document.querySelectorAll('#employeeApp [data-etab]').forEach(b=>{
      const keep = b.dataset.etab === "tipcheck";
      b.classList.toggle("hidden", !keep);
      b.classList.toggle("on", keep);
    });
    document.querySelectorAll('#employeeApp .employeePanel').forEach(p=>p.classList.add("hidden"));
    const checkPanel = $("employeeTipCheckContent")?.closest(".employeePanel") || $("employeeTipCheckContent");
    if(checkPanel) checkPanel.classList.remove("hidden");
    setTimeout(()=>document.querySelector('#employeeApp [data-etab="tipcheck"]')?.click(), 0);
  }
...
// V13.4.1: Employee Check Tip only; Manager/Owner Hourly workflows preserved.
