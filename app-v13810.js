...
// V13.8.10 — reusable Show Password checkbox for all password/PIN inputs.
...
// V13.8.10 reliable team-card click handling (desktop/tablet/mobile).
...
function listenEmployeeHistoricalReports(){
  ...
  unsubs.push(onSnapshot(qh,snap=>{
    ...
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.10 hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.10 Firestore rules.</div>`;
    }
  }
}
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.10 hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.10 payout rule: employee payout is Total Before Meal minus Meal.
...
// V13.8.10 Calculate V01 + fresh page fix.

// V13.8.10: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.10 Employee Clear All fix.

// V13.8.10 Small Report tab + signature XLS.

// V13.8.10 dedicated Hourly Adjustment sub-app.

// V13.8.10 fresh page generated from current index.

// V13.8.10 Edit finalized Hourly report from Small Report.

// V13.8.10 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.10 staff Remember Me with Firebase local/session persistence.

// V13.8.10 Main App button removed from Hourly workspace.

// V13.8.10 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.10 Remember Me for dedicated Hourly login.

// V13.8.10 Small Report server signature + SMS.

// V13.8.10 Employee phone directory for Small Report SMS.

// V13.8.10 recovered employee phone directory + reliable SMS composer.

// V13.8.10 Add/Edit employee + phone directory.

// V13.8.10 Hourly Adjustment V1 batch workspace.

// V13.8.10 login role selector responsive UI fix.

// V13.8.10 Delete Team.

// V13.8.10 Delete individual Hourly V1 team member.

// V13.8.10 Cloud-backed Hourly V1 drafts.

// V13.8.10 live bar calculation after partial Grand Total save.

// V13.8.10 Team BAR Center with AM / 2-4 / PM automation.

// V13.8.10 show password checkboxes.

// V13.8.10 reliable team-card and BAR server input interactions.

// V13.8.10 separate Team Board and employee editor views.

// V13.8.10 persist BAR state to hourlyV1Batches cloud document.

// V13.8.10 secure Clear All, Owner Undo and permanent deletion.

// V13.8.10 BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.10: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.10: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.10 Check Tip soft delete + Owner Undo / Permanent Delete.
