...
async function enableBackgroundPush(){
  ...
  const swReg=await navigator.serviceWorker.register("./service-worker-v13841.js?v=13841",{updateViaCache:"none"});
...
function syncHourlyShift(){
  ...
  if(am) am.classList.toggle("hidden",!(shift==="AM"||shift===SHIFT_EARLY||dbl));
  if(pm) pm.classList.toggle("hidden",!(shift==="PM"||shift===SHIFT_MIDDLE||dbl));
