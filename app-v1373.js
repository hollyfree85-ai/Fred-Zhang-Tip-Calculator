...
function hv1Key(){return HV1_STORAGE_PREFIX+($('hv1Date')?.value||todayLocal())}
function hv1Load(){try{return JSON.parse(localStorage.getItem(hv1Key())||'{"team":[],"drafts":{}}')}catch(e){return {team:[],drafts:{}}}}
function hv1Save(s){localStorage.setItem(hv1Key(),JSON.stringify(s))}
function hv1BlankDraft(name){return {employee:name,date:$('hv1Date')?.value||todayLocal(),values:{},entered:{},skippedPages:[],savedAt:0,finalized:false}}
...
function hv1CapturePage(markSkipped=false){
  ...
  if(hourlyWizardStep===2){
    if(hourlyWizardState.shift==='DOUBLE'){
      [['hAmIn','howAmIn'],['hAmOut','howAmOut'],['hPmIn','howPmIn'],['hPmOut','howPmOut']].forEach(([k,id])=>{const v=hv1VisibleValue(id);if(v!=='')hv1SetValue(d,k,v);});
    }else{[['hIn','howIn'],['hOut','howOut']].forEach(([k,id])=>{const v=hv1VisibleValue(id);if(v!=='')hv1SetValue(d,k,v);});}
  }
  if(hourlyWizardStep===3){hv1SetValue(d,'hBusserAM',hourlyWizardState.busserAM);}
  if(hourlyWizardStep===4){[['hGrandTotal','howGrand'],['hTotalAM','howTotalAM']].forEach(([k,id])=>{const el=$(id);if(el&&String(el.value).trim()!=='')hv1SetValue(d,k,String(el.value).trim())});}
  if(hourlyWizardStep===5){[['hPaidTip','howPaid'],['hCardFee','howCardFee'],['hCashTip','howCash'],['hMeal','howMeal']].forEach(([k,id])=>{const el=$(id);if(el&&String(el.value).trim()!=='')hv1SetValue(d,k,String(el.value).trim())});}
  if(hourlyWizardStep===6){
    ...
    else{hv1SetValue(d,'hAmBar',$('howAmBar')?.checked?'yes':'no');hv1SetValue(d,'hPmBar',$('howPmBar')?.checked?'yes':'no');}
  }
...
function hv1ApplyDraft(name){
  ...
  Object.entries(d.values||{}).forEach(([id,v])=>{if($(id))$(id).value=v});
...
function hv1Enter(){hourlyV1Mode=true;document.body.classList.add('hourly-v1-mode');document.body.classList.remove('hourly-workspace-mode');document.querySelectorAll('.staffPanel').forEach(x=>x.classList.add('hidden'));$('hourlyV1Workspace')?.classList.remove('hidden');if($('hv1Date')&&!$('hv1Date').value)$('hv1Date').value=todayLocal();hv1RenderRoster();const s=hv1Load();$('hv1Setup')?.classList.toggle('hidden',!!s.team?.length);$('hv1BoardBox')?.classList.toggle('hidden',!s.team?.length);if(s.team?.length)hv1RenderCards()}
...
window.hv1DateChanged=function(){hv1RenderRoster();const s=hv1Load();$('hv1Setup').classList.toggle('hidden',!!s.team?.length);$('hv1BoardBox').classList.toggle('hidden',!s.team?.length);if(s.team?.length)hv1RenderCards()};
...
function hv1MarkFinal(){if(!hv1EditingEmployee)return;const s=hv1Load();if(s.drafts?.[hv1EditingEmployee]){s.drafts[hv1EditingEmployee].finalized=true;s.drafts[hv1EditingEmployee].savedAt=Date.now();hv1Save(s)}setTimeout(()=>{document.body.classList.remove('hourly-v1-editing');document.body.classList.add('hourly-v1-mode');$('smallReport')?.classList.add('hidden');$('hourly')?.classList.add('hidden');$('hourlyV1Workspace')?.classList.remove('hidden');hv1RenderCards()},180)}
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.7.3 hard guard: visible Step-6 result stays authoritative.
...
function bindHourlyWizard(){
  ...
  $("howAmBar")?.addEventListener("change",()=>{howSetSilent("hAmBar",$("howAmBar").checked?"yes":"no");renderHourlyWizard();});
  $("howPmBar")?.addEventListener("change",()=>{howSetSilent("hPmBar",$("howPmBar").checked?"yes":"no");renderHourlyWizard();});
...
// V13.7.3 Calculate V01 + fresh page fix.

// V13.7.3: Step-7 no longer erases Bartender Step-6 server totals.

// V13.7.3 Employee Clear All fix.

// V13.7.3 Small Report tab + signature XLS.

// V13.7.3 dedicated Hourly Adjustment sub-app.

// V13.7.3 fresh page generated from current index.

// V13.7.3 Edit finalized Hourly report from Small Report.

// V13.7.3 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.7.3 staff Remember Me with Firebase local/session persistence.

// V13.7.3 Main App button removed from Hourly workspace.

// V13.7.3 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.7.3 Remember Me for dedicated Hourly login.

// V13.7.3 Small Report server signature + SMS.

// V13.7.3 Employee phone directory for Small Report SMS.

// V13.7.3 recovered employee phone directory + reliable SMS composer.

// V13.7.3 Add/Edit employee + phone directory.

// V13.7.3 Hourly Adjustment V1 batch workspace.

// V13.7.3 login role selector responsive UI fix.

// V13.7.3 Delete Team.

// V13.7.3 Delete individual Hourly V1 team member.
