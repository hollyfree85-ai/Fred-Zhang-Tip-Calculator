...
function ownerTableSalesEnabled(row,field){
  ...
  if(field==='total24')return row.role==='Server' && ['DOUBLE','LONG'].includes(row.shift);
...
function ownerTableRender(){
  ...
  const note=group==='sales'?'Cumulative sales: Total 2–4 includes Total AM. Grand Total includes both. Grey fields do not apply to this shift.':group==='clock'?'Use 24-hour time (HH:MM). Double Shift has two clock-in/out pairs.':'Fill this column for everyone, then Save Table. You can switch sections without losing your edits.';
...
async function enableBackgroundPush(){
  ...
  const swReg=await navigator.serviceWorker.register("./service-worker-v13844.js?v=13844",{updateViaCache:"none"});
