...
function bindHourlyTimeMask(){
  ["howIn","howOut","howAmIn","howAmOut","howPmIn","howPmOut"].forEach(id=>{
    const el=$(id);
    if(!el||el.dataset.timeMaskBound==="1")return;
    el.dataset.timeMaskBound="1";
    el.inputMode="numeric";
    el.maxLength=5;
    el.autocomplete="off";
    el.addEventListener("input",()=>{el.value=hourlyFormatTimeDigits(el.value);});
    el.addEventListener("blur",()=>{el.value=hourlyNormalizeTime(el.value);});
  });
}
