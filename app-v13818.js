...
// V13.8.18 — reusable Show Password checkbox for all password/PIN inputs.
...
function hv1ServerBarFees(name,s){
  const state=s||hv1Load();
  hv1EnsureBarState(state);
  ...
  // Grand Totals in BAR are cumulative checkpoints.
  // AM fee = AM cumulative × 0.6%
  const amFee=amGT>0?Math.max(0,amGT*0.006):0;

  // 2–4 fee = 2–4 cumulative × 0.6% − AM fee
  const valid24=gt24>0 && gt24>=amGT;
  const fee24=valid24?Math.max(0,(gt24*0.006)-amFee):0;

  // PM fee = PM cumulative × 0.6% − AM fee − 2–4 fee
  // If 2–4 is blank, compare PM to AM.
  ...
  return {
    amGT,gt24,pmGT,
    amFee,fee24,pmFee,
    totalFee:Math.max(0,amFee+fee24+pmFee),
    valid24,validPM
  };
}
function hv1ServerBarFeeForCheckpoint(key,name,s){
...
function hv1BarReceived(key,s){
  const state=s||hv1Load();
  hv1EnsureBarState(state);
  const names=[...new Set(["AM","2PM_4PM","PM"].flatMap(k=>hv1ServerNamesForCheckpoint(k,state)))];
  return names.reduce((sum,name)=>sum+hv1ServerBarFeeForCheckpoint(key,name,state),0);
}
...
function hv1UpdateBarLive(key,name,value){
  ...
  document.querySelectorAll(`[data-hv1-fee]`).forEach(el=>{
    ...
    const cp=el.dataset.hv1Fee;
    if(emp)el.textContent=fmtMoney(hv1ServerBarFeeForCheckpoint(cp,emp,state));
  });
...
function hv1RenderBarSummaryOnly(state=hv1Load()){
  ...
  host.innerHTML=`<h4>Server Bar Tip Out Summary</h4>
    ...
    ${allServers.map(name=>{
      ...
      return `<div class="hv1-bar-total-row"><b>${esc(name)}</b><span>${fmtMoney(f.amFee)}</span><span>${fmtMoney(f.fee24)}</span><span>${fmtMoney(f.pmFee)}</span><b>${fmtMoney(f.totalFee)}</b></div>`;
...
function hv1ApplyBarAutomation(s){
  ...
  for(const name of s.team||[]){
    ...
    const f=hv1ServerBarFees(name,s);
    const {amGT,gt24,pmGT,amFee,fee24,pmFee,totalFee,valid24,validPM}=f;
...
// V13.8.18 reliable team-card click handling (desktop/tablet/mobile).
...
function listenEmployeeHistoricalReports(){
  ...
  unsubs.push(onSnapshot(qh,snap=>{
    ...
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.18 hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.18 Firestore rules.</div>`;
    }
  }
}
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    alert(`Delete failed: ${e.code||e.message}\n\nIf this says permission-denied, publish the V13.8.18 Firestore rules.`);
  }
};
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.18 hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.18 payout rule: employee payout is Total Before Meal minus Meal.
...
// V13.8.18 Calculate V01 + fresh page fix.

// V13.8.18: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.18 Employee Clear All fix.

// V13.8.18 Small Report tab + signature XLS.

// V13.8.18 dedicated Hourly Adjustment sub-app.

// V13.8.18 fresh page generated from current index.

// V13.8.18 Edit finalized Hourly report from Small Report.

// V13.8.18 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.18 staff Remember Me with Firebase local/session persistence.

// V13.8.18 Main App button removed from Hourly workspace.

// V13.8.18 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.18 Remember Me for dedicated Hourly login.

// V13.8.18 Small Report server signature + SMS.

// V13.8.18 Employee phone directory for Small Report SMS.

// V13.8.18 recovered employee phone directory + reliable SMS composer.

// V13.8.18 Add/Edit employee + phone directory.

// V13.8.18 Hourly Adjustment V1 batch workspace.

// V13.8.18 login role selector responsive UI fix.

// V13.8.18 Delete Team.

// V13.8.18 Delete individual Hourly V1 team member.

// V13.8.18 Cloud-backed Hourly V1 drafts.

// V13.8.18 live bar calculation after partial Grand Total save.

// V13.8.18 Team BAR Center with AM / 2-4 / PM automation.

// V13.8.18 show password checkboxes.

// V13.8.18 reliable team-card and BAR server input interactions.

// V13.8.18 separate Team Board and employee editor views.

// V13.8.18 persist BAR state to hourlyV1Batches cloud document.

// V13.8.18 secure Clear All, Owner Undo and permanent deletion.

// V13.8.18 BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.18: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.18: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.18 Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.18 employee soft-delete ownership uses Firebase UID.

// V13.8.18 finalized edit hydration, autosave, quick Team Board/BAR navigation.

// V13.8.18: direct final editing and two-way BAR/server sync.

// V13.8.18 larger UI, complete signed PDF/SMS, Small Report Grand Total.

// V13.8.18 Grand Total = Total Before Meal + Cash Tip.

// V13.8.18 Small Report fullscreen one-screen viewport + freeze pane + close.

function hideLegacyHourlyV01LoginButtonV13818(){
...
document.addEventListener("DOMContentLoaded",hideLegacyHourlyV01LoginButtonV13818);
setTimeout(hideLegacyHourlyV01LoginButtonV13818,200);

// V13.8.18 consolidated restore: V13.8.14 UI/features + correct cumulative BAR formulas.
