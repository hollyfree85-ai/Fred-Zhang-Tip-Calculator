...
function pdfReportContent(r,index,total){
  ...
  y=702;
  const rightStep=right.length>18?26:35;
...
let bartenderServerRoster=[];

function populateBartenderServerDropdowns(){
  const options=bartenderServerRoster.map(u=>{
  ...
  for(let i=1;i<=9;i++){
    ...
    // Preserve saved/previous selection when refreshing the employee roster.
    if(previous){
...
function listenBartenderServerRoster(){
  unsubs.push(onSnapshot(collection(db,"users"),snap=>{
    bartenderServerRoster=snap.docs
  ...
  },e=>console.error("Bartender server roster:",e)));
}
