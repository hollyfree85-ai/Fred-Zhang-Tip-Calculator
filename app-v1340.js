...
function showApp(){
  ...
  if(emp){
    ...
    // CHECK TIP ONLY build: retain account/admin essentials, remove Tip Report/Hourly workflow.
    const allowed=owner ? ["approvals","users","tipCheck","setup"] : ["approvals","tipCheck","setup"];
    document.querySelectorAll("[data-stab]").forEach(b=>b.classList.toggle("hidden",!allowed.includes(b.dataset.stab)));
    document.querySelectorAll(".ownerOnly").forEach(el=>el.classList.toggle("hidden",!owner));
    listenApprovals();
    if(owner) listenUsers();
    listenTipCheckSheets();
    document.querySelector('[data-stab="tipCheck"]')?.click();
  }
}
...
document.querySelectorAll("[data-stab]").forEach(btn=>{
  btn.addEventListener("click",()=>{
    const role=String(currentProfile?.role||"");
    const allowed=role==="owner"?["approvals","users","tipCheck","setup"]:role==="manager"?["approvals","tipCheck","setup"]:role==="cashier"?["tipCheck","setup"]:[];
    if(!allowed.includes(btn.dataset.stab)) return;
...
window.ownerOpenManualTipCheck=function(){
  if(currentProfile?.role!=="owner")return;
...
function listenUsers(){
  unsubs.push(onSnapshot(collection(db,"users"),snap=>{
    ...
    $("userList").innerHTML=a.map(u=>{
      ...
      const approval=esc(u.approvalStatus||"");
      ...
      <div class="owner-user-card">
        <div class="owner-user-main">
          <div>
            <b style="font-size:18px">${esc(u.displayName||u.username)}</b>
            <div class="small" style="margin-top:3px">${status}</div>
          </div>
          ${action}
        </div>
        <div class="owner-user-grid">
          <div><small>USERNAME</small><b>${esc(u.username||"—")}</b></div>
          <div><small>ROLE</small><b>${esc(u.role||"—")}</b></div>
          <div><small>PHONE</small><b>${phone}</b></div>
          <div><small>APPROVAL</small><b>${approval||"—"}</b></div>
          <div><small>CREATED</small><b>${esc(created)}</b></div>
          <div><small>PIN / PASSWORD</small><b>Protected — not viewable</b></div>
        </div>
      </div>`;
    }).join("");
...
// V13.4.0 Check Tip only + Owner manual entry + expanded User details.
