...
window.calculateBartenderBarTipOut=function calculateBartenderBarTipOut(){
  ...
  if($("hBtFormulaCheck")){
    const label=type==="AM"
      ...
      : type==="2PM_4PM"
        ? `${fmtMoney(summary)} × 0.6% = ${fmtMoney(gross)} − AM ${fmtMoney(lessAM)} = ${fmtMoney(finalReceived)}`
        : `${fmtMoney(summary)} × 0.6% = ${fmtMoney(gross)} − AM ${fmtMoney(lessAM)} − 2-4 ${fmtMoney(less24)} = ${fmtMoney(finalReceived)}`;
