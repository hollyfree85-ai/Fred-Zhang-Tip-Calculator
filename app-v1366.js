...
window.refreshPhoneDirectoryUi=async function(){
  ...
  await refreshEmployeePhoneDirectory(true);
  if(el)el.textContent=`Employee phone directory: ${employeePhoneDirectory.size} number(s) loaded.`;
};
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.6.6 hard guard: visible Step-6 result stays authoritative.
...
// V13.6.6 Calculate V01 + fresh page fix.

// V13.6.6: Step-7 no longer erases Bartender Step-6 server totals.

// V13.6.6 Employee Clear All fix.

// V13.6.6 Small Report tab + signature XLS.

// V13.6.6 dedicated Hourly Adjustment sub-app.

// V13.6.6 fresh page generated from current index.

// V13.6.6 Edit finalized Hourly report from Small Report.

// V13.6.6 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.6.6 staff Remember Me with Firebase local/session persistence.

// V13.6.6 Main App button removed from Hourly workspace.

// V13.6.6 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.6.6 Remember Me for dedicated Hourly login.

// V13.6.6 Small Report server signature + SMS.

// V13.6.6 Employee phone directory for Small Report SMS.
