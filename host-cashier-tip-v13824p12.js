...
function renderShiftRows(shift){
  ...
  host.innerHTML=Array.from({length:7},(_,i)=>{
    ...
    return `<div class="hc-row"><span class="hc-row-num">${i+1}</span>
      ...
      <button class="btn light hc-sign-btn ${signed?"hc-signed":""}" type="button" data-index="${i}" onclick="window.hostCashierSignature?.('${shift}', this.closest('.hc-row')?.querySelector('.hc-employee-select')?.value || '')">${signed?"✓ Signed":"Sign"}</button></div>`;
  }).join("");
...
function openSignature(shift,name){sigTarget={shift,name};sigStrokes=JSON.parse(JSON.stringify(state.signatures?.[shift]?.[name]?.strokes||[]));$("hcSignatureTitle").textContent=`${name} — ${shift} Signature`;$("hcSignatureModal").dataset.opened="1";$("hcSignatureModal").classList.remove("hidden");setTimeout(redrawSig,30)}
...
window.hostCashierSignature=function(shift,name){
  try{
    ...
    openSignature(s,n);
  }catch(e){ console.error("Host/Cashier signature:",e); alert("Signature could not open: "+(e.message||e)); }
};
...
document.addEventListener("click",function(e){
  ...
  if(btn.classList.contains("hc-sign-btn")){
    ...
    const select=row?.querySelector(".hc-employee-select");
    const section=row?.closest(".hc-card");
    const shift=section?.querySelector(".hc-shift-head h3")?.textContent?.includes("PM")?"PM":"AM";
    const name=select?.value||"";
    if(!name){alert("Select an employee first.");return;}
    openSignature(shift,name);
  }
});
