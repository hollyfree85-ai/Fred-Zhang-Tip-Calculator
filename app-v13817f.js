...
// V13.8.17F — reusable Show Password checkbox for all password/PIN inputs.
...
// V13.8.17F reliable team-card click handling (desktop/tablet/mobile).
...
window.openHourlyWorkspacePanel=function(name){
  // V13.8.17FA: Hourly V01 is no longer an isolated sub-app.
...
function listenEmployeeHistoricalReports(){
  ...
  unsubs.push(onSnapshot(qh,snap=>{
    ...
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.17F hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.17F Firestore rules.</div>`;
    }
  }
}
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    alert(`Delete failed: ${e.code||e.message}\n\nIf this says permission-denied, publish the V13.8.17F Firestore rules.`);
  }
};
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.17F hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.17F Busser split. Preserve the existing V01 total busser formula,
  ...
  // V13.8.17F payout rule: employee payout is Total Before Meal minus Meal.
...
// V13.8.17F Calculate V01 + fresh page fix.

// V13.8.17F: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.17F Employee Clear All fix.

// V13.8.17F Daily Report tab + signature XLS.

// V13.8.17F dedicated Hourly Adjustment sub-app.

// V13.8.17F fresh page generated from current index.

// V13.8.17F Edit finalized Hourly report from Daily Report.

// V13.8.17F Daily Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.17F staff Remember Me with Firebase local/session persistence.

// V13.8.17F Main App button removed from Hourly workspace.

// V13.8.17F Daily Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.17F Remember Me for dedicated Hourly login.

// V13.8.17F Daily Report server signature + SMS.

// V13.8.17F Employee phone directory for Daily Report SMS.

// V13.8.17F recovered employee phone directory + reliable SMS composer.

// V13.8.17F Add/Edit employee + phone directory.

// V13.8.17F Tip Calculation batch workspace.

// V13.8.17F login role selector responsive UI fix.

// V13.8.17F Delete Team.

// V13.8.17F Delete individual Tip Calculation team member.

// V13.8.17F Cloud-backed Tip Calculation drafts.

// V13.8.17F live bar calculation after partial Grand Total save.

// V13.8.17F Team BAR Center with AM / 2-4 / PM automation.

// V13.8.17F show password checkboxes.

// V13.8.17F reliable team-card and BAR server input interactions.

// V13.8.17F separate Team Board and employee editor views.

// V13.8.17F persist BAR state to hourlyV1Batches cloud document.

// V13.8.17F secure Clear All, Owner Undo and permanent deletion.

// V13.8.17F BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.17F: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.17F: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.17F Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.17F employee soft-delete ownership uses Firebase UID.

// V13.8.17F finalized edit hydration, autosave, quick Team Board/BAR navigation.

// V13.8.17F: direct final editing and two-way BAR/server sync.

// V13.8.17F larger UI, complete signed PDF/SMS, Daily Report Grand Total.

// V13.8.17F Grand Total = Total Before Meal + Cash Tip.

// V13.8.17F larger Daily Report; frozen Date + Employee columns.

// V13.8.17F simple Daily Report list, detail modal, Busser AM/PM split.

// V13.8.17F Daily Report stays in Tip Calculation; employee detail is a true modal; history retained.

// V13.8.17F Daily Report always visible in Tip Calculation.

// V13.8.17F restores full management tabs when logging in through Hourly V01.

// V13.8.17F professional non-overlapping PDF employee report layout.

// V13.8.17F removes Hourly V01 login and renames Hourly V1 to Tip Calculation.

// V13.8.17F clean role login buttons and simplified staff dashboard.

// V13.8.17F responsive all-device layout; owner credential UI does not expose plaintext Firebase passwords.

// V13.8.17F cashier role/UI removed.
