...
function emp(){
  ...
  ev?.preventDefault?.();
  ev?.stopPropagation?.();
  const had=draft.length>0;
  if(had && !confirm('Clear all unsent tickets?')) return false;
  draft.length=0;
  selected='';
  edit=-1;
  const ticket=$('sxTicket'), tip=$('sxTip'), sel=$('sxSel'), modal=$('sxModal');
  if(ticket) ticket.value='';
  if(tip) tip.value='';
  if(sel) sel.innerHTML='<b>Selected Table:</b> None';
  if(modal) modal.style.display='none';

  // Clear the hidden backing rows directly.
  for(let i=0;i<50;i++){
    const c=$(`eTipCheckCheck${i}`), t=$(`eTipCheckTable${i}`), v=$(`eTipCheckTip${i}`);
    if(c)c.value='';
    if(t)t.value='';
    if(v)v.value='';
  }
