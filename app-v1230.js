...
function moneyReadyRoleShouldSuppressGlobalDialog(){
  return boardMode || currentProfile?.role==="manager" || currentProfile?.role==="owner";
}

async function playBundledMoneyReadyChime(){
...
function trySpeakGlobalMoneyReady(name){
...
async function startGlobalMoneyReadyWatcher(){
  ...
  try{
    ...
    // Anonymous auth is only used when the page is logged out.
    // It does not navigate to Server Room Board and is replaced normally by Employee/Staff login.
    if(!auth.currentUser){
      try{ await signInAnonymously(auth); }catch(e){ console.warn("Global alert anonymous auth:",e); }
    }

    const q=query(collection(db,"moneyReadyBoard"),limit(100));
    globalMoneyReadyUnsub=onSnapshot(q,snap=>{
      const rows=snap.docs.map(d=>({id:d.id,...d.data()}));
      const liveIds=new Set(rows.map(r=>r.id));

      if(!globalMoneyReadyInitialized){
        rows.forEach(r=>globalMoneyReadyKnown.add(r.id));
        globalMoneyReadyInitialized=true;
        return;
      }

      for(const r of rows){
        if(!globalMoneyReadyKnown.has(r.id)){
          globalMoneyReadyKnown.add(r.id);
          if(r.alert!==false){
      ...
      // Forget documents after deletion so a future new document can alert again.
      for(const id of [...globalMoneyReadyKnown]){
        if(!liveIds.has(id)) globalMoneyReadyKnown.delete(id);
      }
    },e=>console.warn("Global Money Ready watcher:",e));
  }catch(e){
    console.warn("Start global Money Ready watcher:",e);
  }
}
...
window.requestNotify=async function(){
  ...
  if("Notification" in window){
    ...
    }catch(e){ permission=Notification.permission||"error"; }
  }

  // Test sound + vibration immediately so Manager/Owner knows the phone is armed.
  try{ navigator.vibrate?.([180,80,180]); }catch(e){}
  realtimeAlertSound("normal");

  alert(
    permission==="granted"
      ? "Sound, vibration, and notifications are enabled on this device while the web app is open."
      : "Sound and vibration are enabled while the web app is open. Browser notification permission: "+permission
  );
};
  ...
  try{
    ...
    if(currentHourlySubmissionId){
      try{
        ...
        // Dialog alert is published first. Server Room card becomes active shortly after.
