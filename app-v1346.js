...
function bindHourlyWizard(){
  $("hourlyOriginalWizard")?.querySelectorAll("[data-how-choice]").forEach(b=>b.onclick=()=>{
    const key=b.dataset.howChoice,val=b.dataset.howValue;
    hourlyWizardState[key]=val;
    if(key==="position"){
      howSet("hPosition",val);
      syncBartenderBarReceivedField();
    }else if(key==="shift"){
  ...
  if($("howNext")) $("howNext").onclick=()=>{
    ...
    if(hourlyWizardStep===7){
      hourlyWizardStep=1;
      ["hIn","hOut","hAmIn","hAmOut","hPmIn","hPmOut","hGrandTotal","hTotalAM","hPaidTip","hCardFee","hCashTip","hMeal"].forEach(id=>{if($(id))$(id).value="0";});
