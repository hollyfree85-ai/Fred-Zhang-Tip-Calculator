...
window.saveTipCheckRow=async function(sheetId,rowIndex){
  ...
  if(!result){alert("Select Done, No Signature, or Ticket Not Found.");return;}
  try{
    await updateTipCheckRowApi({sheetId,rowIndex,result,deleteRow:false});
    await loadTipCheckSheets();
    alert("Row updated.");
  }catch(e){alert(`Row update failed: ${e.message||e.code}`);}
};
