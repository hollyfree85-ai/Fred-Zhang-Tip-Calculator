...
function pdfReportContent(r,index,total){
  ...
  // Footer: keep body safely above this area; signature is isolated on the right.
  c+="0.9 w 28 112 m 584 112 l S\n";

  c+="BT /F2 15 Tf 28 91 Td (TOTAL PAID OUT) Tj ET\n";
  c+=`BT /F2 28 Tf 28 55 Td (${pdfEscape(pdfMoney(r.totalPaidOut))}) Tj ET\n`;

  const status=String(r.status||"MONEY READY").toUpperCase();
  c+="BT /F2 11 Tf 310 93 Td (FINAL REPORT - MONEY READY) Tj ET\n";
  c+=`BT /F1 8.5 Tf 310 79 Td (Status: ${pdfEscape(status)}) Tj ET\n`;
  if(r.finalizedBy){
    c+=`BT /F1 8.5 Tf 310 66 Td (Finalized By: ${pdfEscape(r.finalizedBy)}) Tj ET\n`;
  }
  if(r.pickedUpBy){
    c+=`BT /F1 8.5 Tf 310 53 Td (Picked Up By: ${pdfEscape(r.pickedUpBy)}) Tj ET\n`;
  }
  if(Array.isArray(r.pickupSignature?.strokes) && r.pickupSignature.strokes.length){
    c+="BT /F2 8.5 Tf 310 40 Td (Pickup Signature: SIGNED) Tj ET\n";
  }

  c+="BT /F1 8.5 Tf 438 96 Td (EMPLOYEE PICKUP SIGNATURE) Tj ET\n";
  c+="0.7 w 438 43 m 580 43 l 580 88 l 438 88 l 438 43 l S\n";
  c+=pdfSignatureCommands(r.pickupSignature,442,47,134,37);
...
for(let i=1;i<=9;i++){
  ...
  $(`hBtServerName${i}`)?.addEventListener("change",calculateBartenderBarTipOut);
}
