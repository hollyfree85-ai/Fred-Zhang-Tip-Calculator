...
};sigStrokes=JSON.parse(JSON.stringify(state.signatures?.[shift]?.[name]?.strokes||[]));$("hcSignatureTitle").textContent=`${name} — ${shift} Signature`;$("hcSignatureModal").dataset.opened="1";$("hcSignatureModal").classList.remove("hidden");setTimeout(redrawSig,30)}
