...
function ownerTableEdit(name,field,value){
  ...
  const allowed=['shift',...Object.keys(OWNER_TABLE_MONEY),'total24','barAM','bar24','barPM'];
...
function ownerTableSelectEmployee(name){
  const session=ownerTableSession;
  if(!ownerTableAllowed() || !session || session.uid!==currentUser.uid || ownerTableSaving)return;
  if(!session.rows.some(row=>row.name===name))return;
  session.selectedEmployee=name;
  ownerTableRender();
  $('ownerTableEmployee')?.scrollIntoView({block:'start',behavior:'auto'});
}
window.ownerTableSelectEmployee=ownerTableSelectEmployee;
function ownerTableRender(){
  const session=ownerTableSession,host=$('ownerTableBody');if(!host || !session || !ownerTableAllowed())return;
  if(!session.rows.some(row=>row.name===session.selectedEmployee))session.selectedEmployee=session.rows[0]?.name||'';
  const picker=$('ownerTableEmployee');
  if(picker){
    picker.innerHTML=session.rows.length?session.rows.map(row=>`<option value="${esc(row.name)}" ${row.name===session.selectedEmployee?'selected':''}>${esc(row.name)} — ${row.role}</option>`).join(''):'<option value="">No employees selected</option>';
    picker.value=session.selectedEmployee;picker.disabled=ownerTableSaving || !session.rows.length;
  }
  if($('ownerTablePeriods'))$('ownerTablePeriods').textContent=['AM','2PM_4PM','PM'].map((key,i)=>['BAR AM','BAR 2–4','BAR PM'][i]+': '+(session.bartenders?.[key]||'Not selected')).join(' · ');
  host.innerHTML=session.rows.map(row=>{
    const employee=encodeURIComponent(row.name),server=row.role==='Server';
    const labels={grand:'Grand Total ($) · full day',totalAM:'Total AM ($)',total24:'Total 2–4 ($) · through 4 PM',paid:'Paid Tip ($)',cardFee:'Pay Card Tip Fee ($)',cash:'Cash Tip ($)',meal:'Meal ($)'};
    const money=(key,disabled=false,note='')=>`<td data-label="${labels[key]}"><input type="number" min="0" step="0.01" inputmode="decimal" placeholder="0.00" data-owner-table-field="${key}" data-employee="${employee}" value="${esc(row[key]??'')}" aria-label="${esc(row.name)} ${key}" ${disabled?'disabled':''}>${note?`<small>${note}</small>`:''}</td>`;
    const check=(key,label,disabled=false)=>!server
      ? `<td class="owner-received" data-label="${label.replace('BAR Sales','Bar Tip Out Received')}"><small>${label.replace('BAR Sales','Received')}</small><b>${fmtMoney(row.received?.[({barAM:'AM',bar24:'2PM_4PM',barPM:'PM'})[key]]||0)}</b></td>`
      : `<td class="owner-table-check" data-label="${label}"><input type="checkbox" data-owner-table-field="${key}" data-employee="${employee}" ${row[key]?'checked':''} ${!server||disabled?'disabled':''} aria-label="${esc(row.name)} ${label}"></td>`;
    const amDisabled=row.shift===SHIFT_MIDDLE || row.shift==='PM' || (!server && !['DOUBLE','LONG'].includes(row.shift));
    return `<tr class="${row.name===session.selectedEmployee?'owner-employee-selected':'owner-employee-other'}"><th scope="row"><b>${esc(row.name)}</b><small>${row.role}</small>${!server?'<small>Receives BAR tip out · no server deduction</small>':''}</th><td data-label="Shift"><select data-owner-table-field="shift" data-employee="${employee}" aria-label="${esc(row.name)} shift"><option value="">Select shift</option>${TIP_SHIFTS.map(v=>`<option value="${v}" ${row.shift===v?'selected':''}>${v==='LONG'?'Long Shift':v==='DOUBLE'?'Double Shift':v}</option>`).join('')}</select></td>${money('grand',false,'Includes Total AM and Total 2–4')}${check('barPM','BAR Sales PM',isEarlyShift(row.shift))}${money('totalAM',amDisabled,server && row.shift==='AM'&&!ownerTableLinked24(row)?'Linked to Grand Total':'')}${check('barAM','BAR Sales AM',row.shift===SHIFT_MIDDLE)}${money('total24',!server||row.shift===SHIFT_EARLY,'Includes Total AM')}${check('bar24','BAR Sales 2–4',row.shift===SHIFT_EARLY)}${money('paid')}${money('cardFee')}${money('cash')}${money('meal')}</tr>`;
  }).join('') || '<tr><td colspan="12">No employees selected. Use Employee Board to create today’s team first.</td></tr>';
  document.querySelectorAll('[data-owner-table-field]').forEach(el=>el.addEventListener(el.type==='checkbox'||el.tagName==='SELECT'?'change':'input',()=>{
    const name=decodeURIComponent(el.dataset.employee||'');
    ownerTableEdit(name,el.dataset.ownerTableField,el.type==='checkbox'?el.checked:el.value);
  }));
}
...
window.logout = async function(){
  ownerTableSession=null;++ownerTableRequest;$("ownerTableBody").innerHTML="";if($("ownerTableEmployee"))$("ownerTableEmployee").innerHTML="";
  hv1RestoreSmallReportHome();
...
async function enableBackgroundPush(){
  ...
  const swReg=await navigator.serviceWorker.register("./service-worker-v13843.js?v=13843",{updateViaCache:"none"});
