...
// V13.8.17E — reusable Show Password checkbox for all password/PIN inputs.
...
window.setLoginMode = function(mode,roleHint=""){
  ...
  ["employeeModeBtn","managerModeBtn","cashierModeBtn","ownerModeBtn","hourlyV1ModeBtn","boardModeBtn"].forEach(id=>$(id)?.classList.remove("on"));
  ...
  if(mode==="staff"){
    ...
    if($("staffLoginTitle"))$("staffLoginTitle").textContent=role==="owner"?"Owner Login":role==="cashier"?"Cashier Login":"Manager Login";
    if($("staffLoginHint"))$("staffLoginHint").textContent=role==="owner"?"Full owner access after authentication.":role==="cashier"?"Cashier access after authentication.":"Manager tools and employee approvals.";
  }
  loginMsg("");
};
...
window.hv1DeleteEmployee=function(name){
  ...
  const ok=confirm(
    `Delete ${name} from the Tip Calculation team for ${date}?`+
    (hasProgress?`\n\nTheir unfinished V1 draft/progress for this date will also be removed.`:"")+
    `\n\nFinalized Hourly Reports and Daily Report records will NOT be deleted.`
  );
  if(!ok)return;
...
// V13.8.17E reliable team-card click handling (desktop/tablet/mobile).
...
window.openHourlyWorkspacePanel=function(name){
  // V13.8.17EA: Hourly V01 is no longer an isolated sub-app.
...
function showApp(){
  ...
  if(emp){
    ...
    document.querySelectorAll("[data-stab]").forEach(b=>b.classList.toggle("hidden",b.dataset.stab!=="setup"));
    document.querySelector('[data-stab="setup"]')?.click();
  }else{
    document.querySelectorAll("[data-stab]").forEach(b=>b.classList.remove("hidden"));
    document.querySelectorAll(".ownerOnly").forEach(el=>el.classList.toggle("hidden",!owner));
    listenStaff();
...
function listenEmployeeHistoricalReports(){
  ...
  unsubs.push(onSnapshot(qh,snap=>{
    ...
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.17E hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.17E Firestore rules.</div>`;
    }
  }
}
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    alert(`Delete failed: ${e.code||e.message}\n\nIf this says permission-denied, publish the V13.8.17E Firestore rules.`);
  }
};
...
window.deleteSmallReportRow=async function(reportId){
  ...
  const date=row.date||"";
  if(!confirm(`Delete this Daily Report row for ${who}${date?` (${date})`:""}? This cannot be undone.`)) return;
  ...
  try{
    ...
    console.error("Delete Daily Report row:",e);
...
window.deleteAllSmallReportRows=async function(){
  ...
  const msg=`Delete ALL ${rows.length} Daily Report row(s)${scope?` for ${scope}`:" currently shown"}? This cannot be undone.`;
  ...
  try{
    ...
    currentHourlyReportId="";
    renderSmallReport();
    alert(`${rows.length} Daily Report row(s) deleted.`);
  }catch(e){
    console.error("Delete All Daily Report:",e);
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.17E hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.17E Busser split. Preserve the existing V01 total busser formula,
  ...
  // V13.8.17E payout rule: employee payout is Total Before Meal minus Meal.
...
// V13.8.17E Calculate V01 + fresh page fix.

// V13.8.17E: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.17E Employee Clear All fix.

// V13.8.17E Daily Report tab + signature XLS.

// V13.8.17E dedicated Hourly Adjustment sub-app.

// V13.8.17E fresh page generated from current index.

// V13.8.17E Edit finalized Hourly report from Daily Report.

// V13.8.17E Daily Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.17E staff Remember Me with Firebase local/session persistence.

// V13.8.17E Main App button removed from Hourly workspace.

// V13.8.17E Daily Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.17E Remember Me for dedicated Hourly login.

// V13.8.17E Daily Report server signature + SMS.

// V13.8.17E Employee phone directory for Daily Report SMS.

// V13.8.17E recovered employee phone directory + reliable SMS composer.

// V13.8.17E Add/Edit employee + phone directory.

// V13.8.17E Tip Calculation batch workspace.

// V13.8.17E login role selector responsive UI fix.

// V13.8.17E Delete Team.

// V13.8.17E Delete individual Tip Calculation team member.

// V13.8.17E Cloud-backed Tip Calculation drafts.

// V13.8.17E live bar calculation after partial Grand Total save.

// V13.8.17E Team BAR Center with AM / 2-4 / PM automation.

// V13.8.17E show password checkboxes.

// V13.8.17E reliable team-card and BAR server input interactions.

// V13.8.17E separate Team Board and employee editor views.

// V13.8.17E persist BAR state to hourlyV1Batches cloud document.

// V13.8.17E secure Clear All, Owner Undo and permanent deletion.

// V13.8.17E BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.17E: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.17E: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.17E Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.17E employee soft-delete ownership uses Firebase UID.

// V13.8.17E finalized edit hydration, autosave, quick Team Board/BAR navigation.

// V13.8.17E: direct final editing and two-way BAR/server sync.

// V13.8.17E larger UI, complete signed PDF/SMS, Daily Report Grand Total.

// V13.8.17E Grand Total = Total Before Meal + Cash Tip.

// V13.8.17E larger Daily Report; frozen Date + Employee columns.

// V13.8.17E simple Daily Report list, detail modal, Busser AM/PM split.

// V13.8.17E Daily Report stays in Tip Calculation; employee detail is a true modal; history retained.

// V13.8.17E Daily Report always visible in Tip Calculation.

// V13.8.17E restores full management tabs when logging in through Hourly V01.

// V13.8.17E professional non-overlapping PDF employee report layout.

// V13.8.17E removes Hourly V01 login and renames Hourly V1 to Tip Calculation.

// V13.8.17E clean role login buttons and simplified staff dashboard.

// V13.8.17E responsive all-device layout; owner credential UI does not expose plaintext Firebase passwords.
