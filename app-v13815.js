...
// V13.8.15 — reusable Show Password checkbox for all password/PIN inputs.
...
function hv1BarCardHtml(key,s){
  ...
  return `<h4>${hv1BarCheckpointLabel(key)}</h4>
    ...
    <div class="hv1-bar-row head"><div>Server</div><div>Grand Total</div><div>0.6% Fee</div></div>
    ${names.length?names.map(name=>{
      const v=bar.entries?.[name]??'';
      return `<div class="hv1-bar-row"><b>${esc(name)}</b><input type="number" inputmode="decimal" min="0" step="0.01" placeholder="0.00" data-hv1-bar-gt="${key}" data-employee="${encodeURIComponent(name)}" value="${v===''?'':esc(v)}"><div class="hv1-bar-fee" data-hv1-fee="${key}" data-employee="${encodeURIComponent(name)}">${fmtMoney((Number(v)||0)*0.006)}</div></div>`;
...
function hv1UpdateBarLive(key,name,value){
  ...
  const fee=(Number(value)||0)*0.006;
  document.querySelectorAll(`[data-hv1-fee="${key}"]`).forEach(el=>{
    let emp="";try{emp=decodeURIComponent(el.dataset.employee||"")}catch(e){emp=el.dataset.employee||""}
    if(emp===name)el.textContent=fmtMoney(fee);
  });
...
function hv1ApplyBarAutomation(s){
  ...
  for(const name of s.team||[]){
    ...
    // CUMULATIVE Grand Totals from BAR Center. Each checkpoint calculates THIS PERIOD only.
    // A later cumulative checkpoint can never be lower than an earlier cumulative checkpoint.
    // Invalid/stale checkpoint values are ignored instead of producing a negative tip-out.
    const amFee=amGT>0?Math.max(0,amGT*0.006):0;
    ...
    const totalFee=Math.max(0,amFee+fee24+pmFee);
    ...
    if(!d.entered.hAmBar) d.values.hAmBar=amGT>0?'yes':'no';
...
// V13.8.15 reliable team-card click handling (desktop/tablet/mobile).
...
window.logout = async function(){
  document.body.classList.remove("employee-report-only");
  hv1RestoreSmallReportHome();
...
function showApp(){
  ...
  document.body.classList.toggle("employee-report-only",role==="employee");
  ...
  // Employee uses the normal Tip Report only. Employee Check Tip is removed for now.
  ...
  if(emp){
    ...
    if(shiftBtn){
      ...
      shiftBtn.classList.add("on");
    }
    $("employeeShiftContent")?.classList.remove("hidden");
  }
  ...
  if(emp){
    // Employee portal is read-only for now: finalized My Tip Reports only.
    $("employeeShiftContent")?.classList.remove("hidden");
    $("employeeTipCheckContent")?.classList.add("hidden");
    $("employeeBottom")?.classList.add("hidden");
    listenEmployeeHistoricalReports();
  }else if(cashier){
    // Cashier is intentionally simple: one payout table only.
    document.querySelectorAll("[data-stab]").forEach(b=>b.classList.toggle("hidden",b.dataset.stab!=="cashierPayout"));
    document.querySelectorAll(".staffPanel").forEach(el=>el.classList.add("hidden"));
    $("cashierPayout")?.classList.remove("hidden");
    listenCashierPayoutBoard();
    document.querySelector('[data-stab="cashierPayout"]')?.classList.add("on");
  }else{
    document.querySelectorAll("[data-stab]").forEach(b=>b.classList.remove("hidden"));
    document.querySelectorAll(".ownerOnly").forEach(el=>el.classList.toggle("hidden",!owner));
    listenStaff();
...
window.setEmployeeTab=function(){
  $("employeeShiftContent")?.classList.remove("hidden");
  $("employeeTipCheckContent")?.classList.add("hidden");
  document.querySelectorAll("[data-etab]").forEach(b=>b.classList.toggle("on",b.dataset.etab==="shift"));
  if(currentProfile?.role==="employee") $("employeeBottom")?.classList.add("hidden");
};
...
window.setEmployeeTab=function(){
  $("employeeShiftContent")?.classList.remove("hidden");
  document.querySelectorAll("[data-etab]").forEach(b=>b.classList.toggle("on",b.dataset.etab==="shift"));
  if(currentProfile?.role==="employee") $("employeeBottom")?.classList.remove("hidden");
};
document.querySelectorAll("[data-etab]").forEach(btn=>btn.addEventListener("click",()=>setEmployeeTab(btn.dataset.etab)));
...
if($("eTipCheckRows")) renderTipEntryRows("eTipCheckRows","eTC");
...
function employeeHistoricalReportHtml(r){
  ...
  return `<details class="tip-check-sheet" style="margin-top:10px">
    <summary style="cursor:pointer;display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap">
      <span><b>${esc(r.date||"")} • ${esc(r.shift||"")}</b><br><span class="small">${esc(r.position||"")} • ${signed?"SIGNED":"SIGNATURE PENDING"}</span></span>
      <span style="font-size:20px;font-weight:1000;color:#0a6f43">${fmtMoney(paidOut)}</span>
    </summary>
    <div class="grid3" style="margin-top:14px">
      <div class="kpi"><span>Grand Total</span><b>${fmtMoney(r.grandTotal||0)}</b></div>
      ...
      <div class="kpi"><span>TOTAL PAID OUT</span><b>${fmtMoney(paidOut)}</b></div>
    </div>
    <div class="small" style="margin-top:10px">
      Clock: ${esc(hours.hourInAM||hours.hourIn||"--")} – ${esc(hours.hourOutAM||hours.hourOut||"--")}
      ${(hours.hourInPM||hours.hourOutPM)?` / ${esc(hours.hourInPM||"--")} – ${esc(hours.hourOutPM||"--")}`:""}
    </div>
    <div class="actions" style="margin-top:12px">
      <button class="btn light" type="button" onclick="downloadMyHistoricalReport('${r.id}')">Download My PDF</button>
    </div>
  </details>`;
}
...
function listenEmployeeHistoricalReports(){
  ...
  if(!name)return;
  const qh=query(collection(db,"hourlyReports"),where("employee","==",name),limit(100));
  unsubs.push(onSnapshot(qh,snap=>{
    const rows=snap.docs.map(d=>({id:d.id,...d.data()}));
    window.__employeeHistoricalHourlyReports=rows;
    renderEmployeeHistoricalReports(rows);
  },e=>{
    console.error("Employee historical reports:",e);
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.15 hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    latestDeletedItems=snap.docs.map(d=>({archiveId:d.id,...d.data()})).filter(x=>x.itemType==="check_tip");
    renderOwnerDeletedItems();
  }catch(e){
    console.error("Deleted items load:",e);
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.15 Firestore rules.</div>`;
    }
  }
}
async function archiveCheckTipSheet(sheet){
  ...
  // IMPORTANT: Employee must NOT pre-read a not-yet-existing archive document.
  // Firestore evaluates a read against resource.data, which does not exist yet,
  // and that caused the permission-denied seen in V13.8.15.
  // A visible Check Tip status is not archived yet, so employee can create it directly.
  if(role!=="employee"){
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    console.error("Employee Check Tip archive V13.8.15:",e);
    alert(`Delete archive failed: ${e.code||e.message}\n\nPublish the V13.8.15 Firestore rules if this says permission-denied.`);
    return;
  }

  // Archive succeeded. Remove immediately from the employee screen.
  // Then refresh shared data; a refresh error must not undo or misreport the delete.
  const targetIds=new Set(targets.map(s=>String(s.id||"")));
  latestTipCheckSheets=latestTipCheckSheets.filter(s=>!targetIds.has(String(s.id||"")));
  ...
  try{
    ...
    console.warn("Post-delete refresh V13.8.15:",e);
  }
};
function renderOwnerDeletedItems(){
  if(currentProfile?.role!=="owner")return;
  const list=$("deletedItemsList"),badge=$("deletedItemsBadge");
  const rows=[...(latestDeletedItems||[])].sort((a,b)=>{
    const ta=Number(a.deletedAt?.seconds||0),tb=Number(b.deletedAt?.seconds||0);return tb-ta;
  });
  if(badge)badge.textContent=rows.length;
  if(!list)return;
  list.innerHTML=rows.length?rows.map(x=>{
    const s=x.snapshot||{};
    const count=Number(s.rowCount||s.rows?.length||0);
    const who=x.deletedBy||"";
    ...
    return `<div class="deleted-item-card">
      <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap">
        <div><b style="font-size:18px">${esc(x.employeeName||s.employeeName||"")}</b>
          <div class="deleted-meta">${esc(x.date||s.date||"")} • Check Tip • ${count} line(s)</div>
          <div class="deleted-meta">Deleted by ${esc(who)} (${esc(x.deletedByRole||"")})${when?` • ${esc(when)}`:""}</div>
        </div>
        <div class="actions">
          <button class="btn gold" type="button" onclick="undoDeletedCheckTip('${x.archiveId}')">Undo</button>
          <button class="btn red" type="button" onclick="permanentDeleteCheckTip('${x.archiveId}')">Delete Permanent</button>
        </div>
      </div>
    </div>`;
  }).join(""):'<div class="notice good">No deleted Check Tip records.</div>';
}
...
window.undoAllDeletedCheckTips=async function(){
  if(currentProfile?.role!=="owner")return;
  if(!latestDeletedItems.length){alert("Nothing to undo.");return;}
  if(!confirm(`Undo all ${latestDeletedItems.length} deleted Check Tip record(s)?`))return;
  try{
    for(let i=0;i<latestDeletedItems.length;i+=350){
      const b=writeBatch(db);latestDeletedItems.slice(i,i+350).forEach(x=>b.delete(doc(db,"deletedItems",x.archiveId)));await b.commit();
    }
    await loadTipCheckSheets();alert("All deleted Check Tip records restored.");
  }catch(e){alert(`Undo All failed: ${e.code||e.message}`);}
};
window.permanentDeleteAllCheckTips=async function(){
  if(currentProfile?.role!=="owner")return;
  if(!latestDeletedItems.length){alert("Nothing to permanently delete.");return;}
  if(!confirm(`DELETE PERMANENTLY ALL ${latestDeletedItems.length} deleted Check Tip record(s)?\n\nThis cannot be undone.`))return;
  try{
    for(const x of latestDeletedItems){
      await deleteTipCheckSheetApi({sheetId:x.itemId});
      await deleteDoc(doc(db,"deletedItems",x.archiveId));
    }
    await loadTipCheckSheets();alert("All deleted Check Tip records permanently deleted.");
  }catch(e){alert(`Permanent Delete All failed: ${e.message||e.code}`);}
};
...
let cashierPayoutReports=[];
let cashierPayoutState={checked:{},hidden:{}};

function cashierPayoutPaidOut(r){
  return Math.max(0,Number(r.totalBeforeMeal||0)-Number(r.meal||0));
}

function cashierPayoutStateRef(){
  return doc(db,"cashierPayoutState","main");
}

async function loadCashierPayoutState(){
  try{
    const snap=await getDoc(cashierPayoutStateRef());
    cashierPayoutState=snap.exists()?{checked:{},hidden:{},...(snap.data()||{})}:{checked:{},hidden:{}};
    cashierPayoutState.checked=cashierPayoutState.checked||{};
    cashierPayoutState.hidden=cashierPayoutState.hidden||{};
  }catch(e){
    console.error("Cashier payout state:",e);
    cashierPayoutState={checked:{},hidden:{}};
  }
}

async function saveCashierPayoutState(){
  await setDoc(cashierPayoutStateRef(),{
    checked:cashierPayoutState.checked||{},
    hidden:cashierPayoutState.hidden||{},
    updatedAt:serverTimestamp(),
    updatedByUid:currentUser?.uid||"",
    updatedBy:currentProfile?.displayName||currentProfile?.username||""
  },{merge:true});
...
function renderCashierPayoutBoard(){
  const body=$("cashierPayoutBody");
  if(!body || currentProfile?.role!=="cashier")return;
  const visible=cashierPayoutReports
    .filter(r=>!cashierPayoutState.hidden?.[r.id])
    .sort((a,b)=>{
  ...
  body.innerHTML=visible.length?visible.map(r=>`
    <tr>
      ...
      <td><b>${fmtMoney(cashierPayoutPaidOut(r))}</b></td>
      <td style="text-align:center">
        <input class="cashier-payout-check" type="checkbox" ${cashierPayoutState.checked?.[r.id]?"checked":""}
          onchange="toggleCashierPayoutCheck('${r.id}',this.checked)"
          aria-label="Paid ${esc(r.employee||"")}">
      </td>
    </tr>`).join("")
    : '<tr><td colspan="4" class="small" style="padding:20px;text-align:center">No payout rows.</td></tr>';
}

function listenCashierPayoutBoard(){
  if(currentProfile?.role!=="cashier")return;
  loadCashierPayoutState().then(renderCashierPayoutBoard);
  const qh=query(collection(db,"hourlyReports"),limit(300));
  unsubs.push(onSnapshot(qh,snap=>{
    cashierPayoutReports=snap.docs.map(d=>({id:d.id,...d.data()}));
    renderCashierPayoutBoard();
  },e=>{
    console.error("Cashier payout reports:",e);
    const body=$("cashierPayoutBody");
    if(body)body.innerHTML=`<tr><td colspan="4" class="notice danger">Could not load Cashier Payout: ${esc(e.code||e.message||"permission error")}</td></tr>`;
  }));
  unsubs.push(onSnapshot(cashierPayoutStateRef(),snap=>{
    cashierPayoutState=snap.exists()?{checked:{},hidden:{},...(snap.data()||{})}:{checked:{},hidden:{}};
    cashierPayoutState.checked=cashierPayoutState.checked||{};
    cashierPayoutState.hidden=cashierPayoutState.hidden||{};
    renderCashierPayoutBoard();
  },e=>console.warn("Cashier payout state live:",e)));
}

window.toggleCashierPayoutCheck=async function(reportId,checked){
  if(currentProfile?.role!=="cashier")return;
  cashierPayoutState.checked=cashierPayoutState.checked||{};
  if(checked)cashierPayoutState.checked[reportId]=true;
  else delete cashierPayoutState.checked[reportId];
  renderCashierPayoutBoard();
  try{await saveCashierPayoutState();}
  catch(e){alert(`Checklist save failed: ${e.code||e.message}`);}
};

window.clearCashierPayoutBoard=async function(){
  if(currentProfile?.role!=="cashier")return;
  const visible=cashierPayoutReports.filter(r=>!cashierPayoutState.hidden?.[r.id]);
  if(!visible.length){alert("No payout rows to delete.");return;}
  if(!confirm(`Delete all ${visible.length} row(s) from the Cashier screen?\n\nHourly Adjustment and Small Report data will NOT be deleted.`))return;
  cashierPayoutState.hidden=cashierPayoutState.hidden||{};
  visible.forEach(r=>cashierPayoutState.hidden[r.id]=true);
  renderCashierPayoutBoard();
  try{
    await saveCashierPayoutState();
    alert("Cashier screen cleared.");
  }catch(e){
    alert(`Delete All failed: ${e.code||e.message}`);
  }
};
...
function smallReportPaidOut(r){
  return Number(r.totalBeforeMeal||0)-Number(r.meal||0);
}
...
window.newHourlyEntryFromSmallReport=function(){
  ...
  if(hourlyV1Mode && document.body.classList.contains("hourly-v1-small-report")){
    ...
    alert("Select an employee card on the Hourly V1 Team Board.");
    return;
  }
  ...
  return `<!doctype html><html><head><meta charset="utf-8">
    ...
    <table>
      <thead><tr>
        ...
        <th>Before Meal</th><th>Cash Tip</th><th>Meal</th><th>Total Paid Out</th><th style="width:230px">Signature</th>
      </tr></thead>
      <tbody>
      ${rows.map(r=>{
...
window.sendSmallReportSms=async function(reportId){
  ...
  const rr={...r,totalPaidOut:smallReportPaidOut(r)};
  const blob=simplePdfBlob([rr]);
  const safeName=String(r.employee||"Employee").replace(/[^a-z0-9_-]+/gi,"_");
  const filename=`Fred_Zhang_Tip_Report_${safeName}_${r.date||todayLocal()}.pdf`;
  const msg=`Hi ${r.employee||""}, your detailed signed tip report for ${r.date||""} (${r.shift||""}) is ready. Total Paid Out: ${fmtMoney(smallReportPaidOut(r))}. The PDF has been downloaded on this device; attach that PDF to this message.`;
...
window.renderSmallReport=function(){
  ...
  if(!rows.length){
    body.innerHTML='<tr><td colspan="18" style="padding:24px;text-align:center">No finalized Hourly Adjustment reports for this filter.</td></tr>';
    return;
  }
  ...
  <body>
    <table>
      ...
      <thead><tr>
        ...
        <th>Total Tip Before Meal</th><th>Cash Tip</th><th>Meal</th><th>Total Paid Out</th><th>Signature</th>
      </tr></thead>
      <tbody>${body}</tbody>
    </table>
  </body></html>`;
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.15 hard guard: visible Step-6 result stays authoritative.
...
window.saveHourlyV01=async function(){
  ...
  try{
    ...
    alert(currentHourlyReportId
      ? "Final report updated. Small Report refreshed. Existing report was edited — no duplicate created."
      : "Final approved. Small Report updated. Employee status: MONEY READY.");
...
// V13.8.15 Calculate V01 + fresh page fix.

// V13.8.15: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.15 Employee Clear All fix.

// V13.8.15 Small Report tab + signature XLS.

// V13.8.15 dedicated Hourly Adjustment sub-app.

// V13.8.15 fresh page generated from current index.

// V13.8.15 Edit finalized Hourly report from Small Report.

// V13.8.15 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.15 staff Remember Me with Firebase local/session persistence.

// V13.8.15 Main App button removed from Hourly workspace.

// V13.8.15 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.15 Remember Me for dedicated Hourly login.

// V13.8.15 Small Report server signature + SMS.

// V13.8.15 Employee phone directory for Small Report SMS.

// V13.8.15 recovered employee phone directory + reliable SMS composer.

// V13.8.15 Add/Edit employee + phone directory.

// V13.8.15 Hourly Adjustment V1 batch workspace.

// V13.8.15 login role selector responsive UI fix.

// V13.8.15 Delete Team.

// V13.8.15 Delete individual Hourly V1 team member.

// V13.8.15 Cloud-backed Hourly V1 drafts.

// V13.8.15 live bar calculation after partial Grand Total save.

// V13.8.15 Team BAR Center with AM / 2-4 / PM automation.

// V13.8.15 show password checkboxes.

// V13.8.15 reliable team-card and BAR server input interactions.

// V13.8.15 separate Team Board and employee editor views.

// V13.8.15 persist BAR state to hourlyV1Batches cloud document.

// V13.8.15 secure Clear All, Owner Undo and permanent deletion.

// V13.8.15 BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.15: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.15: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.15 Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.15 employee soft-delete ownership uses Firebase UID.

// V13.8.15 employee Check Tip self-delete rule compatibility fix.

// V13.8.15 employee soft-delete skips forbidden pre-read of missing archive doc.

// V13.8.15 employee deletedItems query matches Firestore read-rule ownership.

// V13.8.15 Employee Check Tip removed; simple Cashier payout table added.

// V13.8.15 preserves full V1 and only hides employee input UI by role.
