...
// V13.8.17H — reusable Show Password checkbox for all password/PIN inputs.
...
// V13.8.17H reliable team-card click handling (desktop/tablet/mobile).
...
window.openHourlyWorkspacePanel=function(name){
  // V13.8.17HA: Hourly V01 is no longer an isolated sub-app.
...
function listenEmployeeHistoricalReports(){
  ...
  unsubs.push(onSnapshot(qh,snap=>{
    ...
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.17H hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.17H Firestore rules.</div>`;
    }
  }
}
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    alert(`Delete failed: ${e.code||e.message}\n\nIf this says permission-denied, publish the V13.8.17H Firestore rules.`);
  }
};
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.17H hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.17H Busser split. Preserve the existing V01 total busser formula,
  ...
  // V13.8.17H payout rule: employee payout is Total Before Meal minus Meal.
...
// V13.8.17H Calculate V01 + fresh page fix.

// V13.8.17H: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.17H Employee Clear All fix.

// V13.8.17H Daily Report tab + signature XLS.

// V13.8.17H dedicated Hourly Adjustment sub-app.

// V13.8.17H fresh page generated from current index.

// V13.8.17H Edit finalized Hourly report from Daily Report.

// V13.8.17H Daily Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.17H staff Remember Me with Firebase local/session persistence.

// V13.8.17H Main App button removed from Hourly workspace.

// V13.8.17H Daily Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.17H Remember Me for dedicated Hourly login.

// V13.8.17H Daily Report server signature + SMS.

// V13.8.17H Employee phone directory for Daily Report SMS.

// V13.8.17H recovered employee phone directory + reliable SMS composer.

// V13.8.17H Add/Edit employee + phone directory.

// V13.8.17H Tip Calculation batch workspace.

// V13.8.17H login role selector responsive UI fix.

// V13.8.17H Delete Team.

// V13.8.17H Delete individual Tip Calculation team member.

// V13.8.17H Cloud-backed Tip Calculation drafts.

// V13.8.17H live bar calculation after partial Grand Total save.

// V13.8.17H Team BAR Center with AM / 2-4 / PM automation.

// V13.8.17H show password checkboxes.

// V13.8.17H reliable team-card and BAR server input interactions.

// V13.8.17H separate Team Board and employee editor views.

// V13.8.17H persist BAR state to hourlyV1Batches cloud document.

// V13.8.17H secure Clear All, Owner Undo and permanent deletion.

// V13.8.17H BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.17H: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.17H: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.17H Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.17H employee soft-delete ownership uses Firebase UID.

// V13.8.17H finalized edit hydration, autosave, quick Team Board/BAR navigation.

// V13.8.17H: direct final editing and two-way BAR/server sync.

// V13.8.17H larger UI, complete signed PDF/SMS, Daily Report Grand Total.

// V13.8.17H Grand Total = Total Before Meal + Cash Tip.

// V13.8.17H larger Daily Report; frozen Date + Employee columns.

// V13.8.17H simple Daily Report list, detail modal, Busser AM/PM split.

// V13.8.17H Daily Report stays in Tip Calculation; employee detail is a true modal; history retained.

// V13.8.17H Daily Report always visible in Tip Calculation.

// V13.8.17H restores full management tabs when logging in through Hourly V01.

// V13.8.17H professional non-overlapping PDF employee report layout.

// V13.8.17H removes Hourly V01 login and renames Hourly V1 to Tip Calculation.

// V13.8.17H clean role login buttons and simplified staff dashboard.

// V13.8.17H responsive all-device layout; owner credential UI does not expose plaintext Firebase passwords.

// V13.8.17H cashier role/UI removed.

// V13.8.17H stock-style Analytics chart page.

// V13.8.17H password-protected per-person delete + finalized report restore.
