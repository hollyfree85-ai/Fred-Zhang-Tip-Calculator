...
// V13.8.8 — reusable Show Password checkbox for all password/PIN inputs.
...
// V13.8.8 reliable team-card click handling (desktop/tablet/mobile).
...
function listenEmployeeHistoricalReports(){
  ...
  unsubs.push(onSnapshot(qh,snap=>{
    ...
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.8 hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile.role==="employee"){
      const name=String(currentProfile.displayName||currentProfile.username||"").trim();
      qd=query(collection(db,"deletedItems"),where("employeeName","==",name),limit(300));
    }else{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.8 Firestore rules.</div>`;
    }
  }
}
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.8 hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.8 payout rule: employee payout is Total Before Meal minus Meal.
...
// V13.8.8 Calculate V01 + fresh page fix.

// V13.8.8: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.8 Employee Clear All fix.

// V13.8.8 Small Report tab + signature XLS.

// V13.8.8 dedicated Hourly Adjustment sub-app.

// V13.8.8 fresh page generated from current index.

// V13.8.8 Edit finalized Hourly report from Small Report.

// V13.8.8 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.8 staff Remember Me with Firebase local/session persistence.

// V13.8.8 Main App button removed from Hourly workspace.

// V13.8.8 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.8 Remember Me for dedicated Hourly login.

// V13.8.8 Small Report server signature + SMS.

// V13.8.8 Employee phone directory for Small Report SMS.

// V13.8.8 recovered employee phone directory + reliable SMS composer.

// V13.8.8 Add/Edit employee + phone directory.

// V13.8.8 Hourly Adjustment V1 batch workspace.

// V13.8.8 login role selector responsive UI fix.

// V13.8.8 Delete Team.

// V13.8.8 Delete individual Hourly V1 team member.

// V13.8.8 Cloud-backed Hourly V1 drafts.

// V13.8.8 live bar calculation after partial Grand Total save.

// V13.8.8 Team BAR Center with AM / 2-4 / PM automation.

// V13.8.8 show password checkboxes.

// V13.8.8 reliable team-card and BAR server input interactions.

// V13.8.8 separate Team Board and employee editor views.

// V13.8.8 persist BAR state to hourlyV1Batches cloud document.

// V13.8.8 secure Clear All, Owner Undo and permanent deletion.

// V13.8.8 BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.8: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.8: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.8 Check Tip soft delete + Owner Undo / Permanent Delete.
