...
function howField(label,id,value,type="text",placeholder=""){
  ...
  const attrs=[
    ...
    isMoney?'inputmode="decimal" min="0" step="0.01"':"",
...
function renderHourlyWizard(){
  ...
  if(hourlyWizardStep===6){
    if(hourlyWizardState.position==="Bartender"){
      body=`<h3 class="how-title">Bar Tip Out Received</h3>
        <p class="how-sub">Enter the server data. The bartender amount is calculated automatically.</p>
        <div class="how-card">
          <div class="how-info">
            <b>Bartender formula</b><br>
            AM = Server Grand Total Summary × 0.6%.<br>
            2 PM–4 PM = Gross 0.6% − previous Bartender AM.<br>
            PM = Gross 0.6% − previous Bartender AM − previous Bartender 2 PM–4 PM.
          </div>
          <div id="howBartenderCalcMount"></div>
        </div>${howNav()}`;
    }else{
      ...
      <div class="how-card">
        <div class="how-summary">
          ...
          ${hourlyWizardState.position==="Bartender"
            ? `<div><small>Bar Tip Out Received</small><b>${howMoney(howVal("hBartenderBarReceived"))}</b></div>`
            : ""}
        </div>
  ...
  if(hourlyWizardStep===6 && hourlyWizardState.position==="Bartender"){
    mountHourlyBartenderCalculator();
  }else{
    parkHourlyBartenderCalculator();
  }
