...
window.hv1DateChanged=async function(){await hv1CloudRestore();hv1RenderRoster();const s=hv1Load();$('hv1Setup').classList.toggle('hidden',!!s.team?.length);$('hv1BoardBox').classList.toggle('hidden',!s.team?.length);if(s.team?.length)hv1RenderCards()};
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.7.5 hard guard: visible Step-6 result stays authoritative.
...
// V13.7.5 Calculate V01 + fresh page fix.

// V13.7.5: Step-7 no longer erases Bartender Step-6 server totals.

// V13.7.5 Employee Clear All fix.

// V13.7.5 Small Report tab + signature XLS.

// V13.7.5 dedicated Hourly Adjustment sub-app.

// V13.7.5 fresh page generated from current index.

// V13.7.5 Edit finalized Hourly report from Small Report.

// V13.7.5 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.7.5 staff Remember Me with Firebase local/session persistence.

// V13.7.5 Main App button removed from Hourly workspace.

// V13.7.5 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.7.5 Remember Me for dedicated Hourly login.

// V13.7.5 Small Report server signature + SMS.

// V13.7.5 Employee phone directory for Small Report SMS.

// V13.7.5 recovered employee phone directory + reliable SMS composer.

// V13.7.5 Add/Edit employee + phone directory.

// V13.7.5 Hourly Adjustment V1 batch workspace.

// V13.7.5 login role selector responsive UI fix.

// V13.7.5 Delete Team.

// V13.7.5 Delete individual Hourly V1 team member.

// V13.7.5 Cloud-backed Hourly V1 drafts.

// V13.7.5 live bar calculation after partial Grand Total save.
