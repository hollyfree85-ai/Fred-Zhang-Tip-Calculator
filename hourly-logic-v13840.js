(function attachFredTipCalculatorLogic(root, factory) {
  ...
  function parseMoney(value, fallback) {
    const safeFallback = arguments.length > 1 && typeof fallback === "number" ? fallback : 0;
  ...
  function normalizeShift(value) {
    ...
    if (shift === "10:45 - 14:00" || shift === "14:00 - 16:00") return shift;
    if (shift === "DOUBLE") return "DOUBLE";
    if (shift === "LONG") return "LONG";
  ...
  function deriveTotals(input) {
    ...
    if (["AM","10:45 - 14:00","14:00 - 16:00"].includes(shift)) {
      totalAM = grandTotal;
    } else if (shift === "PM") {
      totalPM = grandTotal;
    } else {
  ...
  function calculateBusser(input) {
    ...
    } else if (["AM","10:45 - 14:00","14:00 - 16:00"].includes(totals.shift)) {
    ...
    if (["DOUBLE","LONG"].includes(totals.shift) && busserAM === "WITHOUT") {
  ...
  function calculateBarTipOut(input) {
    ...
    if(totals.shift === "14:00 - 16:00"){
      const mid=values.pmBarSales?roundCent(totals.grandTotal*0.006):0;
      return {am:0,pm:mid,total:mid};
    }
  ...
  function calculateHourlyAdjustment(input) {
    ...
    let adjustmentDecision = hasCandidate
      ...
      : "NONE";
    // NONE records that an earlier calculation had no shortfall. A newly
    // positive shortfall needs a decision, including on reopened reports.
    if(hasCandidate && adjustmentDecision === "NONE") adjustmentDecision = "PENDING";

    // No shortfall means no adjustment; unknown/blank decisions stay pending
    // while a real candidate exists.
    if (!hasCandidate) {
