...
function renderTipEntryRows(tbodyId,prefix,rows=[]){
  ...
  tbody.innerHTML=Array.from({length:50},(_,i)=>{
    ...
    return `<tr><td>${i+1}</td>
      ...
      <td><input id="${prefix}Table${i}" value="${esc(r.table||"")}" placeholder="Table"></td>
...
function readTipEntryRows(prefix){
  ...
  for(let i=0;i<50;i++){
    ...
    if(checkNumber||table||tip>0) rows.push({line:i+1,checkNumber,table,tip:Number.isFinite(tip)?tip:0,checked:false});
  }
  return rows;
}
...
async function employeeUidByName(name){
  const snap=await getDocs(query(collection(db,"users"),where("displayName","==",name),limit(1)));
...
window.submitEmployeeTipCheck=async function(){
  ...
  try{
    const ref=doc(collection(db,"tipCheckSheets"));
    await setDoc(ref,{date,employeeUid:currentUser.uid,employeeName:currentProfile.displayName||currentProfile.username||"",rows,rowCount:rows.length,
      totalTip:rows.reduce((s,r)=>s+Number(r.tip||0),0),status:"waiting_cashier",
      submittedByUid:currentUser.uid,submittedByName:currentProfile.displayName||currentProfile.username||"",submittedByRole:"employee",
      createdAt:serverTimestamp(),updatedAt:serverTimestamp()});
    try{await writeAudit("tip_check_submit",ref.id,currentProfile.displayName||"",{date,rowCount:rows.length});}catch(e){}
    clearEmployeeTipCheck();
    alert("Check Tip sheet submitted to Cashier.");
  }catch(e){alert(`Check Tip submit failed: ${e.code||e.message}`);}
};
...
window.submitStaffTipCheck=async function(){
  ...
  if(!rows.length){alert("Enter at least one Check Number / Table / Tip line.");return;}
  try{
    const employeeUid=await employeeUidByName(employeeName);
    const payload={date,employeeUid,employeeName,rows,rowCount:rows.length,totalTip:rows.reduce((s,r)=>s+Number(r.tip||0),0),
      submittedByUid:currentUser.uid,submittedByName:currentProfile.displayName||currentProfile.username||"",submittedByRole:currentProfile.role,updatedAt:serverTimestamp()};
    if(tipCheckEditId && currentProfile.role==="owner"){
      const old=latestTipCheckSheets.find(x=>x.id===tipCheckEditId);
      await updateDoc(doc(db,"tipCheckSheets",tipCheckEditId),{...payload,status:old?.status||"waiting_cashier",editedBy:currentProfile.displayName||currentProfile.username||"",editedAt:serverTimestamp()});
      try{await writeAudit("tip_check_owner_edit",tipCheckEditId,employeeName,{date,rowCount:rows.length});}catch(e){}
      alert("Check Tip sheet updated.");
    }else{
      const ref=doc(collection(db,"tipCheckSheets"));
      await setDoc(ref,{...payload,status:"waiting_cashier",createdAt:serverTimestamp()});
      try{await writeAudit("tip_check_manager_submit",ref.id,employeeName,{date,rowCount:rows.length});}catch(e){}
      alert("Check Tip sheet submitted to Cashier.");
    }
    clearStaffTipCheckForm();
  }catch(e){alert(`Check Tip save failed: ${e.code||e.message}`);}
};
...
function renderEmployeeTipCheckStatus(){
  ...
  const rows=latestTipCheckSheets.filter(r=>r.employeeUid===currentUser.uid).slice(0,10);
  el.innerHTML=rows.length?rows.map(r=>`<div class="tip-check-sheet">
    <div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap"><b>${esc(r.date||"")}</b>
      <span class="status ${r.status==="cashier_completed"?"approved":"pending"}">${tipCheckStatusLabel(r.status)}</span></div>
    <div class="small">${Number(r.rowCount||r.rows?.length||0)} line(s) • Total Tip ${fmtMoney(r.totalTip||0)}</div>
    ${r.status==="cashier_completed"?`<div class="notice good" style="margin-top:8px">Cashier has uploaded your tips into the system. Please check.</div>`:""}
  </div>`).join(""):'<div class="small">No Check Tip sheets yet.</div>';
}
...
function cashierSheetHtml(r){
  ...
  return `<div class="tip-check-sheet" data-cashier-sheet="${r.id}">
    <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap">
      <div><b style="font-size:18px">${esc(r.employeeName||"")}</b><div class="small">${esc(r.date||"")} • ${activeRows.length} line(s) • Total ${fmtMoney(r.totalTip||0)}</div></div>
      <span class="status pending">WAITING CASHIER</span>
    </div>
    <div class="tablewrap" style="margin-top:10px"><table><thead><tr><th>Done</th><th>#</th><th>Check Number</th><th>Table</th><th>Tip</th></tr></thead>
    <tbody>${activeRows.map((x,i)=>`<tr><td><input type="checkbox" class="cashier-line-check" data-index="${i}" ${x.checked?"checked":""}></td><td>${x.line||i+1}</td><td>${esc(x.checkNumber||"")}</td><td>${esc(x.table||"")}</td><td>${fmtMoney(x.tip||0)}</td></tr>`).join("")}</tbody></table></div>
    <div class="actions" style="margin-top:10px"><button class="btn green" type="button" onclick="submitCashierTipCheck('${r.id}')">Submit Completed Checklist</button></div>
  </div>`;
}
function renderCashierTipCheckQueue(){
  const el=$("cashierTipCheckQueue"); if(!el || currentProfile?.role!=="cashier")return;
  const waiting=latestTipCheckSheets.filter(r=>r.status==="waiting_cashier");
  el.innerHTML=waiting.length?waiting.map(cashierSheetHtml).join(""):'<div class="notice good">No Check Tip sheets waiting for Cashier.</div>';
}
window.submitCashierTipCheck=async function(id){
  if(currentProfile?.role!=="cashier"){alert("Cashier login required.");return;}
  const sheet=latestTipCheckSheets.find(r=>r.id===id); if(!sheet)return;
  const wrap=document.querySelector(`[data-cashier-sheet="${id}"]`);
  const checks=[...(wrap?.querySelectorAll(".cashier-line-check")||[])];
  if(!checks.length){alert("No active lines.");return;}
  if(checks.some(c=>!c.checked)){alert("Please check every line before submitting.");return;}
  try{
    const rows=(sheet.rows||[]).map(r=>({...r,checked:true}));
    await updateDoc(doc(db,"tipCheckSheets",id),{rows,status:"cashier_completed",cashierByUid:currentUser.uid,cashierBy:currentProfile.displayName||currentProfile.username||"",cashierAt:serverTimestamp(),updatedAt:serverTimestamp()});
    try{await writeAudit("tip_check_cashier_completed",id,sheet.employeeName||"",{rowCount:rows.length});}catch(e){}
    alert("Checklist completed. Employee will be notified.");
  }catch(e){alert(`Cashier submit failed: ${e.code||e.message}`);}
};

function renderOwnerTipCheckRecords(){
  const el=$("ownerTipCheckRecords"); if(!el || currentProfile?.role!=="owner")return;
  el.innerHTML=latestTipCheckSheets.length?latestTipCheckSheets.map(r=>`<div class="tip-check-sheet">
    <div style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap"><div>
      ...
      <div class="small">${esc(r.date||"")} • ${tipCheckStatusLabel(r.status)} • ${Number(r.rowCount||r.rows?.length||0)} lines • ${fmtMoney(r.totalTip||0)}</div>
      <div class="small">Submitted by ${esc(r.submittedByName||"")} (${esc(r.submittedByRole||"")})${r.cashierBy?` • Cashier: ${esc(r.cashierBy)}`:""}</div>
    </div><div class="actions"><button class="btn light" type="button" onclick="editTipCheckSheet('${r.id}')">Edit</button><button class="btn red" type="button" onclick="deleteTipCheckSheet('${r.id}')">Delete</button></div></div>
  </div>`).join(""):'<div class="small">No Check Tip records.</div>';
}
window.editTipCheckSheet=function(id){
  ...
  tipCheckEditId=id;$("staffTipCheckEditId").value=id;$("sTipCheckMode").value="EDIT EXISTING";$("sTipCheckDate").value=r.date||todayLocal();$("sTipCheckEmployee").value=r.employeeName||"";
  renderTipEntryRows("sTipCheckRows","sTC",r.rows||[]);$("staffTipCheckSubmitBtn").textContent="Save Changes";
  document.querySelector('[data-stab="tipCheck"]')?.click();$("tipCheckManagerBlock")?.scrollIntoView({behavior:"smooth",block:"start"});
};
window.deleteTipCheckSheet=async function(id){
  if(currentProfile?.role!=="owner")return;if(!confirm("Delete this Check Tip sheet?"))return;
  try{const r=latestTipCheckSheets.find(x=>x.id===id);await deleteDoc(doc(db,"tipCheckSheets",id));try{await writeAudit("tip_check_delete",id,r?.employeeName||"",{before:r||null});}catch(e){}}
  catch(e){alert(`Delete failed: ${e.code||e.message}`);}
};
window.clearAllTipCheckSheets=async function(){
  if(currentProfile?.role!=="owner")return;if(!latestTipCheckSheets.length){alert("No Check Tip records.");return;}
  if(!confirm(`CLEAR ALL CHECK TIP RECORDS?\n\nDelete ${latestTipCheckSheets.length} record(s)?`))return;
  try{for(const r of [...latestTipCheckSheets])await deleteDoc(doc(db,"tipCheckSheets",r.id));try{await writeAudit("tip_check_clear_all","ALL","",{count:latestTipCheckSheets.length});}catch(e){}alert("All Check Tip records cleared.");}
  catch(e){alert(`Clear All failed: ${e.code||e.message}`);}
};

function tipCheckExportRows(){
  const out=[];latestTipCheckSheets.forEach(s=>(s.rows||[]).forEach(r=>out.push({date:s.date||"",employee:s.employeeName||"",status:tipCheckStatusLabel(s.status),checkNumber:r.checkNumber||"",table:r.table||"",tip:Number(r.tip||0),checked:r.checked?"YES":"NO",cashier:s.cashierBy||"",submittedBy:s.submittedByName||""})));return out;
}
function tipCheckXlsBlob(){
  const rows=tipCheckExportRows(),escXml=s=>String(s??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");
  const headers=["Date","Employee","Status","Check Number","Table","Tip","Checked","Cashier","Submitted By"];
  const xmlRows=[headers,...rows.map(r=>[r.date,r.employee,r.status,r.checkNumber,r.table,Number(r.tip||0).toFixed(2),r.checked,r.cashier,r.submittedBy])]
...
window.downloadTipCheckXls=function(){if(currentProfile?.role!=="owner")return;if(!latestTipCheckSheets.length){alert("No Check Tip records.");return;}downloadBlob(tipCheckXlsBlob(),`Fred_Zhang_Check_Tip_${todayLocal()}.xls`);};

function tipCheckPdfContent(sheet,index,total){
  const rows=sheet.rows||[];let c="";
  ...
  c+="0.8 w 28 695 m 584 695 l S\nBT /F2 14 Tf 28 674 Td (#) Tj ET\nBT /F2 14 Tf 70 674 Td (Check Number) Tj ET\nBT /F2 14 Tf 260 674 Td (Table) Tj ET\nBT /F2 14 Tf 370 674 Td (Tip) Tj ET\nBT /F2 14 Tf 470 674 Td (Checked) Tj ET\n";
  let y=650;for(let i=0;i<rows.length&&i<28;i++){const r=rows[i];c+=`BT /F1 14 Tf 28 ${y} Td (${i+1}) Tj ET\nBT /F1 14 Tf 70 ${y} Td (${pdfEscape(r.checkNumber||"")}) Tj ET\nBT /F1 14 Tf 260 ${y} Td (${pdfEscape(r.table||"")}) Tj ET\nBT /F1 14 Tf 370 ${y} Td (${pdfEscape(fmtMoney(r.tip||0))}) Tj ET\nBT /F1 14 Tf 470 ${y} Td (${r.checked?"YES":"NO"}) Tj ET\n`;y-=21;}
  c+=`BT /F1 12 Tf 28 28 Td (Report ${index+1} of ${total} | Generated ${pdfEscape(new Date().toLocaleString())}) Tj ET\n`;return c;
}
function tipCheckPdfBlob(){
  const sheets=latestTipCheckSheets,n=sheets.length,font1=3+n*2,font2=font1+1,objects=[],kids=[];objects[1]="<< /Type /Catalog /Pages 2 0 R >>";
  for(let i=0;i<n;i++){const pageObj=3+i*2,contentObj=pageObj+1;kids.push(`${pageObj} 0 R`);const content=tipCheckPdfContent(sheets[i],i,n);objects[pageObj]=`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 ${font1} 0 R /F2 ${font2} 0 R >> >> /Contents ${contentObj} 0 R >>`;objects[contentObj]=`<< /Length ${content.length} >>\nstream\n${content}\nendstream`;}
  objects[2]=`<< /Type /Pages /Kids [${kids.join(" ")}] /Count ${n} >>`;objects[font1]="<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>";objects[font2]="<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold >>";
  let pdf="%PDF-1.4\n",offsets=[0];for(let i=1;i<=font2;i++){offsets[i]=pdf.length;pdf+=`${i} 0 obj\n${objects[i]}\nendobj\n`;}const xref=pdf.length;pdf+=`xref\n0 ${font2+1}\n0000000000 65535 f \n`;for(let i=1;i<=font2;i++)pdf+=String(offsets[i]).padStart(10,"0")+" 00000 n \n";pdf+=`trailer\n<< /Size ${font2+1} /Root 1 0 R >>\nstartxref\n${xref}\n%%EOF`;return new Blob([pdf],{type:"application/pdf"});
}
window.downloadTipCheckPdf=function(){if(currentProfile?.role!=="owner")return;if(!latestTipCheckSheets.length){alert("No Check Tip records.");return;}downloadBlob(tipCheckPdfBlob(),`Fred_Zhang_Check_Tip_${todayLocal()}.pdf`);};

function listenTipCheckSheets(){
  const q=query(collection(db,"tipCheckSheets"),orderBy("createdAt","desc"),limit(300));
  unsubs.push(onSnapshot(q,snap=>{latestTipCheckSheets=snap.docs.map(d=>({id:d.id,...d.data()}));renderEmployeeTipCheckStatus();renderCashierTipCheckQueue();renderOwnerTipCheckRecords();},e=>console.error("Check Tip listener:",e)));
}
