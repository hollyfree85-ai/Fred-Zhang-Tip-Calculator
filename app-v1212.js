...
window.submitPickupSignature=async function(){
  ...
  try{
    ...
    if(pickupModal){ pickupModal.classList.add("hidden"); pickupModal.style.display="none"; }
    pendingPickup=null;
    pickupSignatureStrokes=[];
          ...
          <article style="border-top:1px solid #edf0f4;padding:16px 0">
            <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap">
              ...
              <div class="actions">
                ...
                <button class="btn light js-announce-again" type="button" data-report-id="${esc(r.id)}">Announce Again</button>
                <button class="btn light js-picked-up" type="button"
                  data-report-id="${esc(r.id)}"
                  data-submission-id="${esc(r.sourceSubmissionId||"")}"
...
// V12.1.2: Reliable Final Daily Report actions.
// Use delegated clicks because the report cards are continuously re-rendered by Firestore snapshots.
document.addEventListener("click",e=>{
  const picked=e.target.closest?.(".js-picked-up");
  if(picked){
    e.preventDefault();
    e.stopPropagation();
    const reportId=picked.dataset.reportId||"";
    const submissionId=picked.dataset.submissionId||"";
    const employee=picked.dataset.employee||"";
    window.markMoneyPickedUp(reportId,submissionId,employee);
    return;
  }

  const announce=e.target.closest?.(".js-announce-again");
  if(announce){
    e.preventDefault();
    e.stopPropagation();
    window.republishMoneyReady(announce.dataset.reportId||"");
  }
},true);
