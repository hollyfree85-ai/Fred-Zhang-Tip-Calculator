...
function hv1ApplyDraft(name){
  ...
  if(hv1SyncLongBusserDraft(longState,name)||repaired)localStorage.setItem(hv1Key(),JSON.stringify(longState));
...
async function enableBackgroundPush(){
  ...
  const swReg=await navigator.serviceWorker.register("./service-worker-v13829.js?v=13829",{updateViaCache:"none"});
...
async function writeHourlyFinalForAccount(r,candidateId,submissionId){
  ...
  return runTransaction(db,async transaction=>{
    ...
    const payload={...r,sourceSubmissionId,status:"money_ready",
