...
// V13.8.3 — reusable Show Password checkbox for all password/PIN inputs.
...
function hv1ServerCheckpointFee(name,key,s){
  const am=hv1ServerCumulativeGT(name,'AM',s);
  const p24=hv1ServerCumulativeGT(name,'2PM_4PM',s);
  const pm=hv1ServerCumulativeGT(name,'PM',s);
  if(key==='AM')return Math.max(0,am)*0.006;
  if(key==='2PM_4PM')return Math.max(0,p24-am)*0.006;
  if(key==='PM')return Math.max(0,pm-p24)*0.006;
  return 0;
}
function hv1ServerTotalBarFee(name,s){
  // Final cumulative PM total is authoritative when present.
  // Otherwise use the latest available cumulative checkpoint.
  const pm=hv1ServerCumulativeGT(name,'PM',s);
  const p24=hv1ServerCumulativeGT(name,'2PM_4PM',s);
  const am=hv1ServerCumulativeGT(name,'AM',s);
  const latest=pm>0?pm:(p24>0?p24:am);
  return Math.max(0,latest)*0.006;
}
...
function hv1BarCardHtml(key,s){
  ...
  const formula=key==='AM'
    ? 'AM cumulative Grand Total × 0.6%'
    : key==='2PM_4PM'
      ? 'Cumulative Grand Total through 4 PM × 0.6% − BAR AM received'
      : 'Cumulative Grand Total through PM × 0.6% − BAR AM received − BAR 2–4 received';
...
// V13.8.3 reliable team-card click handling (desktop/tablet/mobile).
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.3 hard guard: visible Step-6 result stays authoritative.
...
// V13.8.3 Calculate V01 + fresh page fix.

// V13.8.3: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.3 Employee Clear All fix.

// V13.8.3 Small Report tab + signature XLS.

// V13.8.3 dedicated Hourly Adjustment sub-app.

// V13.8.3 fresh page generated from current index.

// V13.8.3 Edit finalized Hourly report from Small Report.

// V13.8.3 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.3 staff Remember Me with Firebase local/session persistence.

// V13.8.3 Main App button removed from Hourly workspace.

// V13.8.3 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.3 Remember Me for dedicated Hourly login.

// V13.8.3 Small Report server signature + SMS.

// V13.8.3 Employee phone directory for Small Report SMS.

// V13.8.3 recovered employee phone directory + reliable SMS composer.

// V13.8.3 Add/Edit employee + phone directory.

// V13.8.3 Hourly Adjustment V1 batch workspace.

// V13.8.3 login role selector responsive UI fix.

// V13.8.3 Delete Team.

// V13.8.3 Delete individual Hourly V1 team member.

// V13.8.3 Cloud-backed Hourly V1 drafts.

// V13.8.3 live bar calculation after partial Grand Total save.

// V13.8.3 Team BAR Center with AM / 2-4 / PM automation.

// V13.8.3 show password checkboxes.

// V13.8.3 reliable team-card and BAR server input interactions.

// V13.8.3 separate Team Board and employee editor views.

// V13.8.3 persist BAR state to hourlyV1Batches cloud document.

// V13.8.3 secure Clear All, Owner Undo and permanent deletion.

// V13.8.3 Small Report remains inside Hourly V1 workspace.

// V13.8.3 cumulative BAR grand-total logic.
