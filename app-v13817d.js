...
// V13.8.17D — reusable Show Password checkbox for all password/PIN inputs.
...
// V13.8.17D reliable team-card click handling (desktop/tablet/mobile).
...
window.openHourlyWorkspacePanel=function(name){
  // V13.8.17DA: Hourly V01 is no longer an isolated sub-app.
...
function listenEmployeeHistoricalReports(){
  ...
  unsubs.push(onSnapshot(qh,snap=>{
    ...
    const host=$("employeeHistoricalReports");
    if(host){
      host.innerHTML=`<div class="notice danger"><b>Could not load My Tip Reports.</b><br>${esc(e.code||e.message||"Firestore permission error")}<br><span class="small">Publish the V13.8.17D hourlyReports employee-read rule.</span></div>`;
    }
  }));
}
...
async function loadDeletedItems(){
  ...
  try{
    ...
    if(currentProfile?.role==="owner"&&$("deletedItemsList")){
      $("deletedItemsList").innerHTML=`<div class="notice danger">Deleted / Undo could not load: ${esc(e.code||e.message)}. Publish the V13.8.17D Firestore rules.</div>`;
    }
  }
}
...
window.employeeDeleteTipCheckDate=async function(encodedDate){
  ...
  try{
    ...
    alert(`Delete failed: ${e.code||e.message}\n\nIf this says permission-denied, publish the V13.8.17D Firestore rules.`);
  }
};
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.8.17D hard guard: visible Step-6 result stays authoritative.
  ...
  // V13.8.17D Busser split. Preserve the existing V01 total busser formula,
  ...
  // V13.8.17D payout rule: employee payout is Total Before Meal minus Meal.
...
// V13.8.17D Calculate V01 + fresh page fix.

// V13.8.17D: Step-7 no longer erases Bartender Step-6 server totals.

// V13.8.17D Employee Clear All fix.

// V13.8.17D Daily Report tab + signature XLS.

// V13.8.17D dedicated Hourly Adjustment sub-app.

// V13.8.17D fresh page generated from current index.

// V13.8.17D Edit finalized Hourly report from Daily Report.

// V13.8.17D Daily Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.8.17D staff Remember Me with Firebase local/session persistence.

// V13.8.17D Main App button removed from Hourly workspace.

// V13.8.17D Daily Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.8.17D Remember Me for dedicated Hourly login.

// V13.8.17D Daily Report server signature + SMS.

// V13.8.17D Employee phone directory for Daily Report SMS.

// V13.8.17D recovered employee phone directory + reliable SMS composer.

// V13.8.17D Add/Edit employee + phone directory.

// V13.8.17D Tip Calculation batch workspace.

// V13.8.17D login role selector responsive UI fix.

// V13.8.17D Delete Team.

// V13.8.17D Delete individual Tip Calculation team member.

// V13.8.17D Cloud-backed Tip Calculation drafts.

// V13.8.17D live bar calculation after partial Grand Total save.

// V13.8.17D Team BAR Center with AM / 2-4 / PM automation.

// V13.8.17D show password checkboxes.

// V13.8.17D reliable team-card and BAR server input interactions.

// V13.8.17D separate Team Board and employee editor views.

// V13.8.17D persist BAR state to hourlyV1Batches cloud document.

// V13.8.17D secure Clear All, Owner Undo and permanent deletion.

// V13.8.17D BAR source-of-truth + payout report + signed PDF/SMS.

// V13.8.17D: no negative BAR fees; PM checkbox can be manually unchecked.

// V13.8.17D: exact-recipient SMS, employee historical reports, finalized V1 edit-in-place.

// V13.8.17D Check Tip soft delete + Owner Undo / Permanent Delete.

// V13.8.17D employee soft-delete ownership uses Firebase UID.

// V13.8.17D finalized edit hydration, autosave, quick Team Board/BAR navigation.

// V13.8.17D: direct final editing and two-way BAR/server sync.

// V13.8.17D larger UI, complete signed PDF/SMS, Daily Report Grand Total.

// V13.8.17D Grand Total = Total Before Meal + Cash Tip.

// V13.8.17D larger Daily Report; frozen Date + Employee columns.

// V13.8.17D simple Daily Report list, detail modal, Busser AM/PM split.

// V13.8.17D Daily Report stays in Tip Calculation; employee detail is a true modal; history retained.

// V13.8.17D Daily Report always visible in Tip Calculation.

// V13.8.17D restores full management tabs when logging in through Hourly V01.

// V13.8.17D professional non-overlapping PDF employee report layout.

// V13.8.17D removes Hourly V01 login and renames Hourly V1 to Tip Calculation.

// V13.8.17D clean role login buttons and simplified staff dashboard.
