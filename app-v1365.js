...
onAuthStateChanged(auth, async user=>{
  ...
  try{
    ...
    if(hourlyWorkspaceRequested && !["manager","owner"].includes(String(profile.role||""))){
      hourlyWorkspaceRequested=false;
...
function findEmployeeUserByName(name){
  ...
  if(!key)return null;
  return latestUsers.find(u=>
    String(u.displayName||"").trim().toLowerCase()===key ||
...
window.sendSmallReportSms=async function(reportId){
  ...
  // If this Hourly report came from an employee submission, use the
  // existing server-side Twilio resend function.
  if(r.sourceSubmissionId){
    try{
      ...
      // Fall through to native SMS composer if the employee profile has a phone.
    }
  }

  const user=findEmployeeUserByName(r.employee);
  const phone=String(user?.phone||"").replace(/[^\d+]/g,"");
  if(!phone){
    alert(`No mobile phone is saved for ${r.employee||"this employee"}. Add the phone number in Owner → Users.`);
    return;
  }
      ...
      <td>
        <div class="actions" style="flex-wrap:wrap;min-width:330px">
          ...
          <button class="btn light" type="button"
            onclick="sendSmallReportSms('${r.id}')">SMS</button>
...
window.calculateHourlyV01=function(){
  ...
  if(isBartenderNow && bartenderWizardSnapshot){
    ...
    // V13.6.5 hard guard: visible Step-6 result stays authoritative.
...
function howBartenderServerOptions(slot,selected=""){
  ...
  const names=[...EMPLOYEE_ROSTER].filter(n=>n!==current).sort((a,b)=>a.localeCompare(b));
...
// V13.6.5 Calculate V01 + fresh page fix.

// V13.6.5: Step-7 no longer erases Bartender Step-6 server totals.

// V13.6.5 Employee Clear All fix.

// V13.6.5 Small Report tab + signature XLS.

// V13.6.5 dedicated Hourly Adjustment sub-app.

// V13.6.5 fresh page generated from current index.

// V13.6.5 Edit finalized Hourly report from Small Report.

// V13.6.5 Small Report + New Entry.

setTimeout(loadStaffRememberPreference,0);

// V13.6.5 staff Remember Me with Firebase local/session persistence.

// V13.6.5 Main App button removed from Hourly workspace.

// V13.6.5 Small Report delete row/delete all/PDF.

setTimeout(loadHourlyRememberPreference,0);

// V13.6.5 Remember Me for dedicated Hourly login.

// V13.6.5 Small Report server signature + SMS.
