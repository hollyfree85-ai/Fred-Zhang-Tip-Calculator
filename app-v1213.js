...
window.preparePickupSignatureFromDirect=function(reportId,submissionId,employee){
  pendingPickup={reportId,submissionId,employee};
  pickupSignatureStrokes=[];
  initPickupSignatureCanvas();
  setTimeout(redrawPickupSignature,50);
};
...
window.submitPickupSignature=async function(){
  if(!pendingPickup){
    const modal=$("pickupSignatureModal");
    if(modal?.dataset?.reportId){
      pendingPickup={
        reportId:modal.dataset.reportId||"",
        submissionId:modal.dataset.submissionId||"",
        employee:modal.dataset.employee||""
      };
    }
  }
  if(!pendingPickup){
    alert("Pickup information is missing. Please close and try Picked Up again.");
    return;
  }
            ...
            <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap">
              ...
              <div class="actions">
                ...
                <button class="btn light" type="button"
                  onclick="openPickupSignatureDirect('${r.id}','${r.sourceSubmissionId||""}',this.dataset.employee)"
...
// V12.1.3: direct DOM Picked Up opener; service worker disabled/unregistered; cache hard reset.
